...
void doit(uint64_t k, uint64_t d, uint64_t ed, mpz_t *res, mpz_t *W, mpz_t *Wk)
{
...
int main(int argc, char **argv)
{
  ...
  for(uint64_t d=d0;d<=d1;d++)
	...
	mpz_mul_ui(edk,edk,ed);
      ...
      doit(k,d,ed,&res,W,Wk);
      //printf("doit returned ");mpz_out_str(NULL,10,res);printf("\n");
      //fmpz_get_mpz(res1,res);
      mpfi_mul_z(temp_i,this_kd,res);
      //fmpz_get_mpz(res1,edk);
      mpfi_div_z(this_kd,temp_i,edk);
      mpfi_mul_2exp(temp_i,this_kd,1);
      //if((d%100)==0)
      //{
	  printf("Result for %lu was ",d);
