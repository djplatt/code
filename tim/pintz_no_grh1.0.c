...
void init_twopims()
{
  ...
  //fmprb_init(c);
  //fmprb_set_ui(temp,109);
  //fmprb_div_ui(c,temp,154,PREC);
  ...
  for(m=0;m<=k;m++)
    {
      ...
      fmprb_add_fmpr(twopims[m],temp1,lambda,PREC);
      fmprb_sin_cos(sintwopims[m],costwopims[m],twopims[m],PREC);
...
void make_U()
{
  ...
  fmprb_add(temp1,temp,c,PREC);
  fmprb_div_fmpr(temp,temp1,lambda,PREC);

  printf("(psi(%f)+%f)/%f<=",lambdad,109.0/154.0,lambdad);
  fmprb_printd(temp,10);
  printf("\n");
...
int main(int argc, char**argv)
{
  ...
  k=atol(argv[5]);
  fmpr_init(lambda);
  uint64_t num,fin=atol(argv[2]),den=atol(argv[3]);
  init_twopims();
  fmprb_init(c);
  fmprb_set_ui(temp,109);
  fmprb_div_ui(c,temp,154,PREC);

  for(num=atol(argv[1]);num<=fin;num++)
    {
