...
Uses the first 5000 zeros of zeta to estimate sum 0<gam<inf gam^(-2)
and gam^(-3) where the gammas are the imaginary parts of the zeta zeros
...
int main()
{
  ...
  for(it=0;it<num_its;it++)
      ...
      for(z=zs[0]+1;z<=zs[1];z++)
	{
	  ...
	  mpfi_sqr(del_t,tmp);
	  mpfi_div(del_t,one,del_t);
	  mpfi_add(res,res,del_t);
	  mpfi_div(del_t,del_t,tmp);
	  mpfi_add(res1,res1,del_t);
	}
    }
  mpfi_set_d(t,st[1]);
  mpfi_log(del_t,t);
  mpfi_div_d(t,del_t,st[1]);
  mpfi_add(res,res,t);
  mpfi_div_d(t,t,st[1]);
  mpfi_add(res1,res1,t);
  mpfi_print_str("Sigma gam^-2 is less than ",res);
  mpfi_print_str("Sigma gam^-3 is less than ",res1);
}
