...
G1(a,b,c)={local(t1,t2,t3,res,n);t1=log(2.001^2*b*c);t2=log(1.994*sqrt(a)/sqrt(b));t3=log(1.994^2*a*c);n=floor(solve(nn=2,1000000,(nn+1)/nn-(t1+t2/nn)/t3));return((t1+t2/n)/t3);}
...
repl=0;pass=0;fails=0;p=1;for(i=1,length(X)/4,a=X[p];p++;b=X[p];p++;c=X[p];p++;d=X[p];p++;g1=G1(a,b,c);g2=G2(a,b,g1);g3=G3(a,c,g1,g2);if(!border(a,b,d,g3),fails++,pass++;print(pass,": ",fails," : ( ",a, " , ",b, " , ",c, " , ",d," ) is still a possible.")));print("We had ",fails," failures.");print("We had ",repl," replacements.");
