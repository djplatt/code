 ...
 * Uses Fuj result that d=d+=a+b+c+2abc+2rst
 *
...
uint64_t check_ds(uint64_t a, uint64_t b, uint64_t ur, mpz_t *ztmps, mpfi_t *itmps)
{
  uint64_t i,res=0;
  ...
  for(i=0;i<cptr;i++)
	...
	continue;
      if((i>0)&&(mpz_cmp(cs[i],cs[i-1])==0))
	continue;
      //printf("a=%lu b=%lu c=",a,b);mpz_out_str(NULL,10,cs[i]);printf("\n");
      mpz_add_ui(ztmps[0],cs[i],a+b); // a+b+c
      //printf("a+b+c=");mpz_out_str(NULL,10,ztmps[0]);printf("\n");
      mpz_mul_ui(ztmps[1],cs[i],a);
      mpz_add_ui(ztmps[3],ztmps[1],1);
      mpz_sqrt(ztmps[2],ztmps[3]); // s
      mpz_mul_ui(ztmps[3],ztmps[1],2*b); // 2abc
      //printf("2abc=");mpz_out_str(NULL,10,ztmps[3]);printf("\n");
      mpz_add(ztmps[1],ztmps[3],ztmps[0]); // a+b+c+2abc
      mpz_mul_ui(ztmps[0],ztmps[2],2*ur); // 2rs
      mpz_mul_ui(ztmps[2],cs[i],b);
      mpz_add_ui(ztmps[2],ztmps[2],1);
      mpz_sqrt(ztmps[3],ztmps[2]); // t
      mpz_mul(ztmps[2],ztmps[3],ztmps[0]); // 2rst
      //printf("2rst=");mpz_out_str(NULL,10,ztmps[2]);printf("\n");
      mpz_add(ztmps[0],ztmps[1],ztmps[2]); // a+b+c+2abc+2rst
      if(mpz_cmp(ztmps[0],upb)>0) // d too big, don't test the larger c's
	return(res);
      if(mpz_cmp(ztmps[0],lwb)>=0)
	{
	  if(lara(ztmps[0],itmps))
	    {
	      printf("Solution found %lu %lu ",a,b);
	      mpz_out_str(NULL,10,cs[i]);printf(" ");
	      mpz_out_str(NULL,10,ztmps[0]);printf("\n");
	      res++;
	    }
	  else
	    {
	      //printf("Rejecting %lu %lu ",a,b);
	      //mpz_out_str(NULL,10,cs[i]);printf(" ");
	      //mpz_out_str(NULL,10,ztmps[0]);printf("\n");
	      bad_d_count++;
	    }
...
void set_cs(uint64_t x, uint64_t y, mpz_t u, mpz_t v, uint64_t D, uint64_t a, uint64_t b1, mpz_t *ztmps, uint64_t b, uint64_t ur, mpfi_t *itmps)
{
  //printf("in set_cs with a=%lu b=%lu x=%lu y=%lu u=",a,b,x,y);
  ...
  mpz_set_ui(y_n,y);
  uint64_t res=0;
  int good_c=set_c(xn,a,c,b1); // 1 if c is good
  //printf("c=");mpz_out_str(NULL,10,c);printf("\n");
  while(mpz_cmp_ui(c,b)<=0) // c<=b
    {
      ...
      good_c=set_c(xn,a,c,b1);
      //printf("c=");mpz_out_str(NULL,10,c);printf("\n");
    }
  while(mpz_cmp(c,upb)<=0) // c<=b^8
    {
      if(good_c)
	{
	  if(cptr==C_LIM)
	    {
	      printf("Not enough space to store values of c. Exiting.\n");
	      exit(0);
	    }
	  //printf("adding new c=");mpz_out_str(NULL,10,c);printf("\n");
	  mpz_set(cs[cptr++],c);
	}
      mpz_mul(ztmps[0],xn,u);
      mpz_mul_ui(ztmps[2],y_n,D);
...
{
  uint64_t g=gcd(b,a),a1=a/g,b1=b/g,D=a1*b1,N=b1*(b1-a1),res=0;
  cptr=0;
  ...
  if(PQm1(pm1res,D,sqrt_D,itmps,rtmp,ztmps[0])==-1)
    {
      ...
      // square this solution to get X^2-Dy^2=1
	  ...
	  if(x%b1==0) // x is an integer
	    {
	      ...
	      set_cs(x,y,pm1res[0],pm1res[1],D,a,b1,ztmps,b,ur,itmps);
  ...
  return(check_ds(a,b,ur,ztmps,itmps));
}
