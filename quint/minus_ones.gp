...
cs=vector(C_LIM);
ds=vector(D_LIM);
generate_cs(u,v,x0,y0,D,a,b,b1)=
{
   local(b2,c,lwb,upb,x,y);
   /*print("In generate_cs with ",u," ",v," ",x0," ",y0," ",a," ",b," ",b1," ",D);*/
   x=x0;y=y0;
   upb=b^8;
   b2=b1*b1;
   c=(x*x/b2-1)/a;
...
generate_cs_ds_extra(u,v,x0,y0,D,a,b,b1)=
{
   ...
   while(c<lwb,
      ...
      x=t;
      c=(x*x/b2-1)/a);
   for(i=1,cptr-1,if(type(cs[i])=="t_INT",print("c found=",cs[i])));
   d=c;
   while(d<=upb,
      ds[dptr]=[d];
      dptr++;
...
lara(a,b,d)=d^0.125/4/sqrt(a)<log(4.001*a*b^2*d)*log(1.299*sqrt(a*b)*d/(b-a))/log(4*b*d)/log(0.1053*d/b/(b-a)^2);

/* test every c>b */
test_ds(r,a,b)={
   local(c,d,scs,lwb,upb);
   lwb=b^5;
   upb=b^8;
   scs=vecsort(cs,1,8);
   for(i=1,length(scs),
      c=scs[i][1];
      if(c>b,
         d=a+b+c+2*a*b*c+2*r*floor(sqrt(a*c+1))*floor(sqrt(b*c+1));
         if(d>lwb,
           if(d>upb,
              return,
              nsols++;
...
minus_one(r,a,b)={
   ...
   m1=PQm1(D,sD);\\
   if(m1[1]==-1,
      /*printf("X^2-%d y^2=-%d has a solution.\n",D,N);
      print("m1=",m1);*/
      L1=ceil(sqrt(N/D));
      t=m1[2]*m1[2]+m1[3]*m1[3]*D;
      L2=floor(sqrt(N*(t+1)/(2*D)));
      /*printf("L1=%d L2=%d\n",L1,L2);*/
      for(y=L1,L2,Sq=D*y^2-N;
         if(issquare(Sq),
            x=floor(sqrt(Sq));
            /*printf("Solution to X^2-%dy^2=-%d found (%d,%d).\n",D,N,x,y);*/
            generate_cs(m1[2],m1[3],x,y,D,a,b,b1);
            generate_cs(m1[2],-m1[3],x,y,D,a,b,b1)));
      t=m1[2]^2+m1[3]^2*D;
      m1[3]*=2*m1[2];
      m1[2]=t);
...
test_minus_ones(X)={nsols=0;sols=0;for(ptr=1,length(X)/2,a=X[ptr*2-1];b=X[ptr*2];r=floor(sqrt(a*b+1));minus_one(r,a,b));printf("We came up with %d solutions out of %d.\n",sols,nsols);}

X=readvec("~/data/quint/minus_ones.log");

printf("Processing %d exceptions.\n",length(X)/2);
...
new_minus_one(r,a,b)={
   ...
   sD=sqrt(D);\\
   m1=PQm1(D,sD);\\
   if(m1[1]!=-1,print("Error, ",a," ",b," did not have a -1 solution."));
   if(m1[1]==-1,
      /*printf("X^2-%d y^2=-%d has a solution.\n",D,N);
      print("m1=",m1);*/
      L1=ceil(sqrt(N/D));
      ...
      for(y=L1,L2,Sq=D*y^2-N;
         if(issquare(Sq),
            ...
            if(w<0,w=-w);
            if(w!=1,
              print(a," ",b," Genuine extra solution found ",x," ",y," ",m1[2]," ",m1[3])))))}


new_test_minus_ones(X)={nsols=0;sols=0;for(ptr=1,length(X)/2,a=X[ptr*2-1];b=X[ptr*2];r=floor(sqrt(a*b+1));new_minus_one(r,a,b));printf("We came up with %d solutions out of %d.\n",sols,nsols);}
