...
#define d_log2 (mpfi_log_2)
mpfi_t d_small,a,b,lambda,r;
mpfi_c_t c_small;

#define MAX_LOGS (10000)

mpfi_t logns[MAX_LOGS];

void setup()
{
  mpfi_init(d_small);
  mpfi_interv_d(d_small,-LN_SMALL,LN_SMALL);
  mpfi_c_init(c_small);
  mpfi_set(c_small->re,d_small);
  mpfi_set(c_small->im,d_small);
  int n;
  for(n=0;n<MAX_LOGS;n++)
    {
      mpfi_init(logns[n]);
    }
  mpfi_set_ui(logns[1],0);
  for(n=2;n<MAX_LOGS;n++)
    {
      mpfi_set_ui(logns[0],n);
      mpfi_log(logns[n],logns[0]);
    }
  mpfi_clear(logns[0]);
  mpfi_init(a);mpfi_init(b);mpfi_init(lambda);mpfi_init(r);
}

//#define TARGET_ACC (40)
// error from truncating sum of F_hat to M terms

inline void F_hat_o_err(mpfi_c_ptr res, mpfi_ptr x,const unsigned int q, mpfi_ptr logq, unsigned int *M, mpfi_ptr delta, mpfi_ptr logdelta, mpfi_ptr pi_sindelta_by_q)
{
  //int_double a,b;
  ...
  mpfi_mul_d(a,logq,0.75);
  mpfi_sub(b,d_log2,a);
  mpfi_mul_d(a,x,1.5);
  mpfi_add(b,b,a);
  //print_int_double_str("b=",b);
  //print_int_double_str("lambda=",lambda);
  double d_b=mpfi_get_d(b),d_a;
  if(d_b>TARGET_ACC)
    M[0]=1;
  else
    {
      d_a=sqrt((TARGET_ACC-b)/mpfi_get_d(lambda));
      M[0]=(unsigned int) d_a;
      M[0]++;
    }
