...
g++ checkm.cpp -ocheckm -O2 -frounding-math -fomit-frame-pointer -finline-functions -mfpmath=387 -I$CRDIR -L$CRDIR -lcrlibm
...
main(int argc, char *argv[])
{
  ...
  for(n0=0; n0<=N; n0+=M) {
    ...
    fillmublock(mu,isprime,n0,M+v+1);
  ...
  printf("For x\\leq %ld\n",N);
  for(j=1; j<=2; j++)
    printf("\t|T_%d(x)|\\leq %g/x + %.10g/sqrt(x)\n",j,-bar[j].right,
