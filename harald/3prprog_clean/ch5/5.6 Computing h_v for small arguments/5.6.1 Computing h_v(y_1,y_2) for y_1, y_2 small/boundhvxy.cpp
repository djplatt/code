...
main(int argc, char *argv[])
{
  ...
  printf("The following bounds concern f_{%ld,%d}(x), defined as\n",rat,v);
  printf("\t inf_{r x <= y <= %ld} sqrt(x y) h_%d(x,y) for r=%ld.\n",N,v,rat);
  printf("Then, for 1<=x<=%ld/r, where r=%ld,\n",N,rat);
  printf("f_{%ld,%d}(x) >= %.10g\n",rat,v,lowh);
  printf("and f_{%ld,%d}(x)/x >= %.10g; that value may be reached near x=%.10g\n",rat,v,minrat,minratthere);
  printf("The integral of f_{%ld,%d}/x^2 from 0 to infty is at least %.10g\n",rat,v,integ.left);
  printf("The integral of min(0,f_{%ld,%d})/x^2 from 0 to infty is at least %.10g\n",rat,v,neginteg.left);
  printf("Define F_{%ld,%d}(x) = max_{1<=t<=x} (-f_{%ld,%d}(t)).\n",rat,v,rat,v);
  printf("Then the integral of F_{%ld,%d}(x)/x^2 from 0 to infty is at most  %.10g\n",rat,v,-mininteg.left);
}
