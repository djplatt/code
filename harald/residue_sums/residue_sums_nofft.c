...
void do_rho(arb_t gamma, acb_t zetap, arb_t t0, int64_t prec)
{
  static acb_t s,ctmp1,z;
  static arb_t tmp1,t1,t2,varp,den,inv_s_abs,inv_zetap;
  static bool init=false;
  if(!init)
    {
      init=true;
      acb_init(s);acb_init(ctmp1);acb_init(z);
  ...
  acb_abs(inv_zetap,zetap,prec); // |1/zeta'|
  max_min_zetap(inv_zetap,gamma,prec);
...
int main(int argc, char **argv)
{
  ...
  while(true)
    {
      if(arb_load_file(gamma,infile))
	break;
      //printf("gamma = ");arb_printd(gamma,20);printf("\n");
      arb_sub(tmp,gamma,a_t0,prec);
      if(arb_is_positive(tmp))
	break;
      if(!arb_is_negative(tmp))
	{
	  printf("Could not determine sign of gamma-t0. Exiting.\n");
	  exit(0);
	}
      z_count++;      
      if(arb_load_file(acb_realref(zetap),infile))
	{
	  printf("Error reading real part of 1/zeta'. Exiting.\n");
	  exit(0);
	}
      if(arb_load_file(acb_imagref(zetap),infile))
	{
	  printf("Error reading imaginary part of 1/zeta'. Exiting.\n");
	  exit(0);
	}
      do_rho(gamma,zetap,a_t0,prec);
    }
