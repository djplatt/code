...
int main(int argc, char **argv)
{
  ...
  ptype *target,tl,tl2;
  ptype i,j,count,target_len=XI*SEGS_PER_SIEVE;
  bigint target_start,del2_sum;
  FILE *infile,*outfile;
  ptype num_its,sn;
  if(argc!=5)
    fatal_error("Usage <primes file> <sieve no> <num its> <outfile>"); 
  ...
  */
  ...
  for(i=0,target_start=1;i<LOG_10_X0;i++,target_start*=10);
  target_start+=1; // must be odd
  target_start-=NUM_SIEVES/2*target_len;
  target_start+=target_len*sn;
  ...
  for(j=0;j<num_its;j++,target_start+=target_len)
    { 
      printf("Sieving from ");print_bigint(target_start);printf("\n");
      set_primes(target,target_len);
      sieve(target,target_start,target_len,infile);

      for(tl2=XI>>1,tl=XI,i=1;i<=target_len;tl2+=XI,tl+=XI)
	{
	  printf("from ");
