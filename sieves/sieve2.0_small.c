...
inline void sieve1 (ptype *target, bigint target_start, const ptype target_len, primegen *pg)
{
  ...
  ptype q,ptr,two_q,max_p=exp(log(target_end)/3.0);

  // we can't trust double prec at large height
  // so just use it as a guide
  /*
  printf("max_p was %ld\n",max_p);
  while((bigint) max_p*max_p*max_p<target_end)
    max_p++;
  while((bigint) max_p*max_p*max_p>target_end)
    max_p--;
  */
...
int main(int argc, char **argv)
{
  ...
  for(it=sieve_num;it<num_its+sieve_num;it++,target_start+=target_len)
	...
	if(primep(i,target))
	  count++;
      printf("Sieving to x^1/3 left %lu potential primes.\n",count);
      exit(0);
