...
#define PRIMORIAL ((ptype) 2*3*5*7*11*13*17*19)
#define NUM_PRIMES ((ptype) 8)
#define TWO_PRIMORIAL ((ptype) PRIMORIAL<<1)
#define NEXT_PRIME ((ptype) 23)
ptype PRIMES[NUM_PRIMES]={2,3,5,7,11,13,17,19};
...
inline void sieve1 (ptype *target, bigint target_start, const ptype target_len, primegen *pg)
{
  ...
  ptype p,max_p=sqrt(target_end),ptr,two_p,i,del;
  ...
  // clear out multiples of primes that make up PRIMORIAL
  ...
  // now clear out multiples of pseudoprimes (p,PRIMORIAL)=1, p>=NEXT_PRIME
  for(del=NEXT_PRIME;del<=PRIMORIAL+1;del+=2)
    if(co_prime(del,PRIMORIAL))
      {
	printf("Doing equivalence class %lu\n",del);
	for(p=del;p<=max_p;p+=PRIMORIAL)
	  {
	    two_p=p<<1;
	    ptr=get_index(p,target_start);
	    while(ptr<=target_len)
