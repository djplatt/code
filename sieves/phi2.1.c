		  ...
		  mpfi_ptr err, mpfi_ptr x, mpfi_ptr t0, ptype xi)
{
  ...
  mpfi_set_ui(ttmp,xi); // xi
  mpfi_neg(texp,ttmp);
  mpfi_put(ttmp,texp); // [-xi,xi]
  mpfi_add(ta3,ttmp,t0); // [t0-xi,t0+xi]
  mpfi_div(tlogs[0],ta3,x);
  mpfi_log(tlogs[0],tlogs[0]);
  mpfi_sqr(tlogs[1],tlogs[0]);
  mpfi_mul(tlogs[2],tlogs[1],tlogs[0]);
  mpfi_mul_d(texp,tlogs[1],-0.5);
  mpfi_div(texp,texp,lambdas[1]); // -log^2/2/lam^2
  mpfi_exp(texp,texp); // exp(-log^2/2/lam^2)

  mpfi_div_ui(ttmp,t0,xi); // t0/xi
  mpfi_sub_ui(ttmp,ttmp,1);
  mpfi_sqr(ttmp,ttmp);
  mpfi_sqr(ttmp,ttmp); //(t0/xi-1)^4
  mpfi_mul(ttmp,ttmp,root2pi);
  mpfi_mul(ttmp,ttmp,lambdas[2]);
  mpfi_mul_ui(ttmp,ttmp,24);
  mpfi_div(texp,texp,ttmp);

  mpfi_div(ttmp,tlogs[2],lambdas[4]); // log^3/lam^4

  mpfi_sub_ui(ta3,tlogs[0],1);
  mpfi_mul_ui(ta3,ta3,3);
  mpfi_mul(ta3,ta3,tlogs[0]);
  mpfi_div(ta3,ta3,lambdas[1]);
  mpfi_add(ttmp,ttmp,ta3); // + 3log*(log-1)/lam^2

  mpfi_mul_ui(ta3,tlogs[0],11);
  mpfi_add(ttmp,ttmp,ta3); // +11 log

  mpfi_sub_ui(ta3,lambdas[1],1);
  mpfi_mul_ui(ta3,ta3,6);
  mpfi_add(ttmp,ttmp,ta3); // +6(lam^2-1)

  mpfi_mul(texp,texp,ttmp); // exp(-..)*....

  mpfi_neg(ttmp,texp);
  mpfi_put(texp,ttmp);
  ifdebug(mpfi_print_str("Additional Taylor error=",texp));
  mpfi_add(err,err,texp);
...
int main(int argc, char **argv)
{
  ...
  mpfi_set_ui(res,0);
  mpfi_set_128(x,xb);
  printf("lambda=%40.38e\n",LAMBDA);mpfi_print_str("x  =",x);printf("File=%s\n",argv[1]);
