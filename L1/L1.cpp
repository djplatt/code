  ...
  if(pr)  // q has a primitive root so nice and easy 1-d FFT
  {
      ...
      for(i=0;i<phi_q;i++)
	{
      ...
      */
      ...
      // now fft_vec contains L(chi,s)*q
      /*
      for(i=0;i<phi_q;i++)
	{
      ...
      exit(0);
      */
      for(i=1,k=0;i<phi_q;i++)  // i=0 corresponds to principal chi, only do odd ones
        if(prim_p(i,factors[q].primes[0]))        
	  {
	    if(conj_j(k,n_prims,q)>=k)
	      {
		x=mod(Fft_vec[i]/q);
		check_low(q,i,x/log_q);
		check_high(q,i,x*log_q);
	      }
	    k++;
	  }
  }
...
int main(int argc, char **argv)
{
  ...
  */
  ...
  /*  
  sort_heap(low_left,&low_left[HEAP_LEN],llcomp);
  print_heap(low_left,HEAP_LEN);
  printf("\n");
  sort_heap(low_right,&low_right[HEAP_LEN],lrcomp);
  print_heap(low_right,HEAP_LEN);
  printf("\n");
  sort_heap(high_left,&high_left[HEAP_LEN],hlcomp);
  print_heap(high_left,HEAP_LEN);
  printf("\n");
  sort_heap(high_right,&high_right[HEAP_LEN],hrcomp);
  print_heap(high_right,HEAP_LEN);
  */
  long unsigned int len=HEAP_LEN;
  fwrite(&len,sizeof(long unsigned int),1,out_file);
  fwrite(low_left,sizeof(node),HEAP_LEN,out_file); // contains large L(1)/log(q) based on left end point
  fwrite(low_right,sizeof(node),HEAP_LEN,out_file); // contains large L(1)/log(q) based on right end point
  fwrite(high_left,sizeof(node),HEAP_LEN,out_file); // contains small L(1)*log(q) based on left end point
  fwrite(high_right,sizeof(node),HEAP_LEN,out_file); // contains small L(1)*log(q) based on right end point
  for(i=0;i<10;i++){print_node(low_left[i]);printf("\n");
  print_node(low_right[i]);printf("\n");
  print_node(high_left[i]);printf("\n");
  print_node(high_right[i]);printf("\n");}
