...
Version 1.4: Only processes even primitive characters with chi(2)=1
...
void make_l(unsigned int q, 
	    ...
	    bool *indices)
{
  bool touched=false; // set to true if we do anything
  ...
  int_double smallest_even,largest_even;
  ...
  if(pr)  // q has a primitive root so nice and easy 1-d FFT
  {
    ...
    for(i=1;i<phi_q;i+=2)
      indices[i]=false;
    for(i=0;i<phi_q;i+=2) // only even characters 
      if((prim_p(i,factors[q].primes[0]))&&(contains_zero(Fft_vec[i]-c_one))) // only primitive and chi(2)==1
	{
	  indices[i]=true;
	  touched=true;
	}
    ...
    if(!touched)
      {
	//printf("No qualifying characters mod %u\n",q);
	return;
      }
    for(i=1;i<q;i++)
      if(co_prime(i,q))
    ...
    for(i=1,k=0;i<phi_q;i++)  // i=0 corresponds to principal chi
      {
	if(indices[i]) // only do even primitive charcters whith chi(2)==1
	  {
	    //printf("k=%lu, conj(k)=%lu\n",k,conj_j(k,n_prims,q));
	    if(conj_j(k,n_prims,q)>=k) // pair conjugates
	      {
		//print_int_complex_str("z=",Fft_vec[i]/q);
		x=mod(Fft_vec[i])/q;
    ...
    printf("q=%u: ",q);print_int_double(smallest_even);print_int_double_str(" ",largest_even);
  }
  ...
  else // q doesn't have a primitive root
  {
    ...
    for(i=0,k=0;i<phi_q;i++)
      {
	indices[i]=false;
	if(conj_j(k,n_prims,q)>=k) // only do one of conjugate pairs
	  {
	    primitive=true;
	    ...
	    if(primitive) // only do primitive characters
	      {
		//printf("Index %u primitive.\n",i);
		neg_one=neg_one_p(coords,factors[q].num_facs);
		if(power_2&&(i<(phi_q>>1)))
		  neg_one=!neg_one;
		//if(!neg_one) printf("Character %u is even.\n",i);
		if((!neg_one)&&contains_zero(Fft_vec[i]-c_one)) // only do even characters with chii(2)==1
		  {
		    touched=true;
		    indices[i]=true;
		  }
...
int main(int argc, char **argv)
{
  ...
  */
  ...
  bool * indices;
  if(!(indices=(bool *) calloc(q_end-1,sizeof(bool))))
    fatal_error("Error allocating memory for indices. Exiting.\n");
  ...
  */
  for(q=q_start;q<=q_end;q++)
	...
	make_l(q,num_s,factors,q_n,im_s_vec,offset_vec,a_n,omegas,out_file,indices);
	fflush(stdout);
      }
