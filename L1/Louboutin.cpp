	  ...
	  if(prim_p(i,factors[q].primes[0])) // only do primitive charcters
	    {
	      ...
	      if(conj_j(k,n_prims,q)>k) // pair conjugates, ignore reals
		{
		  int_double y;
		  y=mod(Fft_vec[i])/q;
		  x=y*log_q;
		  if(x.left<smallest.left)
		    {
		      smallest=x;
		      printf("New possible smallest found with q = %d ",q);
		      print_int_double_str("",smallest);
		    }
		  y/=log_q;
		  if(y.right<biggest.right)
		    {
		      biggest=y;
		      printf("New possible biggest found with q = %d ",q);
		      print_int_double_str("",biggest);
		    }
...
int main(int argc, char **argv)
{
  ...
  */
  ...
  for(q=q_start;q<=q_end;q++)
    if((q&3)!=2) // no primitive characters here
      {
  ...
  printf("%s successful completion on q=%d-%d file %s\n",argv[0],q_start,q_end,argv[3]);
