...
checks all even primitive characters whith chi(2)=1
|L(1,chi)|<=1/2 log(q)-0.02012
...
long unsigned int prims_done=0;

int_double odd_bound,even_bound;

void save_data(unsigned int q, int_double *results, FILE *outfile)
{
  if(fwrite(&q,sizeof(unsigned int),1,outfile)==1)
    if(fwrite(results,sizeof(int_double),2,outfile)==2)
      return;
  printf("Error writing data. Exiting.\n");
  exit(0);
}

void process_delta(unsigned int q, unsigned int i, int_double &delta, int_double &bound, int_double *sofar)
{
  printf("q=%u ",q);
  print_int_double_str("delta=",delta);

  if(delta.left>-bound.right)
    {
      printf("Exception found with q=%u index=%u delta=",q,i);
      print_int_double(delta);
      print_int_double_str(" bound=",bound);
    }
  if(delta.left<sofar[0].left)
    sofar[0].left=delta.left;
  if(delta.right<sofar[0].right)
    sofar[0].right=delta.right;
}
      ...
      FILE *outfile)
{
  ...
  int_double log_q_2,x,delta;
  log_q_2=log(int_double(q));
  log_q_2/=2;
#define MAX_DELTA (100)
  int_double results[2];
  // these are illegal intervals [100,-100]
  results[0].left=MAX_DELTA;
  results[0].right=MAX_DELTA;
  results[1].left=MAX_DELTA;
  results[1].right=MAX_DELTA;
  ...
  if(pr)  // q has a primitive root so nice and easy 1-d FFT
  {
      ...
      for(i=1,k=0;i<phi_q;i++)  // i=0 corresponds to principal chi
	{
	  ...
	  if(!(x<even_bound))
	    {
	      printf("q %u: index %u: |L(1,chi)|-1/2 log q=",q,i);
	  ...
	  if(prim_p(i,factors[q].primes[0])) // only do primitive charcters
		...
		{
		  if(i&1) continue; // odd characters 
		  print_int_complex_str("L(1,chi)=",Fft_vec[i]/q);
		  x=mod(Fft_vec[i])/q;
		  delta=x-log_q_2;
		  //print_int_double_str("     |L(1,chi)|-1/2log(q)=",delta);
		  //process_delta(q,i,delta,even_bound,results+1);
		}
	      k++;
	    }
	  */
	}
      save_data(q,results,outfile);
  }
...
int main(int argc, char **argv)
{
  ...
  even_bound=int_double(-2012)/100000;
  ...
  */
  ...
  printf("l-func successful completion on q=%d-%d file %s, primitive characters processed=%lu\n",q_start,q_end,argv[3],prims_done);
