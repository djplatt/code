...
checks all primitive characters = 0 mod 3
|L(1,chi)|<=1/3 log q + 0.3682 (even)
                      + 0.8383 (odd)
      ...
      FILE *outfile)
{
  ...
  int_double log_q_3=log(int_double(q))/3,x,delta;
	  ...
	  if(prim_p(i,factors[q].primes[0])) // only do primitive charcters
	      ...
	      if(conj_j(k,n_prims,q)>=k) // pair conjugates
		{
		  ...
		  if(i&1) // its an odd character
		    process_delta(q,i,delta,odd_bound,results);
		  else
...
int main(int argc, char **argv)
{
  ...
  even_bound=int_double(3682)/10000;
  odd_bound=int_double(8383)/10000;
