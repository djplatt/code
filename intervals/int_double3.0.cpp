...
unsigned long _nze[2]={0,0x80000000}; // -0.0

double nze = *(double*)_nze;
  ...
  /* assumes that intervals are in correct order */
{
  ...
  */
  ...
  switch(mask)
    ...
    case 0:
      left=right/rhs.left;
      right=nextafter(left/rhs.right);
      return(*this);
    case 1:
      printf("Division by interval containing zero.\n");
      exit(0);_fpu_restore;
...
inline int_double int_double::operator/= (const int rhs)
{
  ...
  printf("Attempt to divide interval by int 0. Exiting.");
  exit(0);_fpu_restore;
  return(*this);
};
