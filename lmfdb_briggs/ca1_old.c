...
void check_Robin(arb_t lhs, arb_t rhs, int64_t prec)
{
  static int init=false;
  ...
  arb_sub(tmp1,lhs,tmp2,prec);
  if(!arb_is_negative(tmp1))
    {
      arb_exp(tmp3,rhs,prec); // rhs contains log n, print n
      printf("n = ");arb_printd(tmp3,30);printf("\n");
...
int main(int argc, char **argv)
{
  ...
  while(fscanf(infile,"%lu %lu %lu\n",&p1,&p2,&e)==3)
    {
      if(ca.next_entry==ca.n_entries) // just realloc here!
	{
  ...
  eps_ext(ca.entries+ca.next_entry,prec);
  eps_inc(ca.entries+ca.next_entry-1,prec);
  ...
  for(uint64_t n=0;n<100000000;n++)
      ...
      if(n%1000000==0)
	{
      ...
      check_Robin(lhs,rhs,prec);
      uint64_t ptr=find_biggest(ca.entries,ca.next_entry,prec);
      //printf("Chosen entry is ");print_entry(ca.entries+ptr);
