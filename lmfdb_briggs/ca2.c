...
arb_t log_stack[STACK_LEN];
uint64_t log_ptr=0;
arb_t sig_stack[STACK_LEN];
uint64_t sig_ptr=0;

void add_item(arb_t *stack, arb_t res, arb_t item, uint64_t k, uint64_t *ptr, int64_t prec)
{
  arb_set(stack[ptr[0]++],item);
  while(k&1)
    {
      arb_add(stack[ptr[0]-2],stack[ptr[0]-2],stack[ptr[0]-1],prec);
      ptr[0]--;
      k>>=1;
    }
  arb_set(res,stack[0]);
...
int check_Robin(arb_t lhs, arb_t rhs, int64_t prec, int verbosep)
{
  static int init=false;
  ...
  if(verbosep)
    {printf("log log n = ");arb_printd(tmp1,20);printf(" exp(gamma) log log n = ");arb_printd(tmp2,20);printf(" lhs-rhs=");arb_printd(tmp3,20);printf("\n");fflush(stdout);}
...
void include_prime(entry_t *entry, uint64_t p, arb_t log_p, arb_t lhs, arb_t rhs, uint64_t k, int64_t prec)
{
  static int init=false;
  ...
  entry->p2=p;
  add_item(log_stack,rhs,log_p,k,&log_ptr,prec); // log(n*p) = log n + log p
  
  arb_set_ui(tmp1,p);
  arb_div(tmp2,lhs,tmp1,prec);
  add_item(sig_stack,lhs,tmp2,k,&sig_ptr,prec);
  //arb_add(lhs,lhs,tmp2,prec);
  /*
  arb_set_ui(tmp1,p+1);
...
// update lhs and rhs
void add_exp(entry_t *entry, arb_t lhs, arb_t rhs, uint64_t k, int64_t prec)
{
  static int init=false;
  static arb_t tmp,tmp1,tmp2;
  if(!init)
  ...
  entry->e++;
  add_item(log_stack,rhs,entry->log_p1,k,&log_ptr,prec);
  eps_max(entry,prec);
  /*
  ...
  */
  ...
  arb_pow_ui(tmp1,tmp,entry->e+1,prec); // p^(e+2)
  arb_sub_ui(tmp,tmp1,entry->p1,prec); // p^(e+2)-p
  arb_set_ui(tmp1,entry->p1-1); // p-1
  arb_div(tmp2,tmp1,tmp,prec); // (p-1)/(p^(e+2)-p)
  arb_mul(tmp,lhs,tmp2,prec);
  add_item(sig_stack,lhs,tmp,k,&sig_ptr,prec);
  //arb_add(lhs,lhs,tmp,prec);

}

// replace sigma(p^e)/p^e with sigma(p^e+1)/p^e+1
void arb_sig_del(arb_t lhs, uint64_t p, uint64_t e, uint64_t k, int64_t prec)
{
  static int init=false;
  ...
  arb_sub_ui(tmp1,tmp2,p,prec); // p^(e+2)-p
  arb_set_ui(tmp2,p-1);
  arb_div(tmp3,tmp2,tmp1,prec);
  arb_mul(tmp1,lhs,tmp3,prec);
  add_item(sig_stack,lhs,tmp1,k,&sig_ptr,prec);
}
...
int main(int argc, char **argv)
{
  ...
  for(uint64_t ptr=0;ptr<STACK_LEN;ptr++)
    {
      arb_init(log_stack[ptr]);
      arb_init(sig_stack[ptr]);
    }
  ...
  for(uint64_t n=1;n<=n_its;n++)
      ...
      if(!rc)
	{
      ...
      ptr=find_biggest(ca.entries,ca.next_entry,prec); 

      if(ca.entries[ptr].p1==ca.entries[ptr].p2) // just increment exponent
	{
	  add_exp(ca.entries+ptr,lhs,rhs,n,prec);
	  continue;
	}
      if(ptr==ca.next_entry-1) // need a new prime, add it to the last proper entry
	{

	  include_prime(ca.entries+ptr-1,ca.entries[ptr].p1,ca.entries[ptr].log_p1,lhs,rhs,n,prec);
	  tp=primesieve_next_prime(&it);
	...
	{
	  add_item(log_stack,rhs,ca.entries[ptr].log_p1,n,&log_ptr,prec); // increase log(n)
	  arb_sig_del(lhs,ca.entries[ptr].p1,ca.entries[ptr].e,n,prec);
