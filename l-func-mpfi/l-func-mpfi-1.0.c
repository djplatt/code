...
Build instructions: gcc -ol-func-mpfi-1.0 l-func-mpfi-1.0.c -O2 -lmpfi -lmpfr -lgmp -lm
...
int N,M,RN,N_SUM,NO_GAPS,N_OUT,RN_OUT; 
/*
N = width of lattice
...
void print_usage()
{
  printf("Usage: l-func-mpfi <prec> <N> <M> <RN> <N_SUM> <NO_GAPS> <data file> <out file> <N_OUT> <RN_OUT>\n");
  printf("N>=M\nRN >=2\nN_SUM > RN\n");
...
// called with hi_prec[j] set to zeta(1/2+j+it), j=0..N
// assumes rn>=2
// sets up top and bottom row of lattice
...
int process_s(FILE *infile, FILE *outfile, int num_zeta)
{
    ...
    /*
    // write Im(s)/2 mod 2pi to file
    mpfi_set_d(taylor_err,s.im/2.0);
    mpfi_mod(taylor_err,taylor_err,mpfi_2_pi);
    write_float(outfile,taylor_err);
    */

// two values of gamma function, just read and output
// not needed here but used in next phase
    read_c(infile,z);  // Gamma(s/2)
	mpfi_const_pi(taylor_err);
	mpfi_mul_d(taylor_err,taylor_err,s.im/4.0);
	mpfi_exp(taylor_err,taylor_err);
	mpfi_mul(z,z,taylor_err);
    printf("gamma value ");mpfi_c_print(z);printf("\n");
    ...
    /*
    for(i=0;i<N_OUT;i++)
      printf(" %d",-mpfi_rel_error(r_n_vals[N*NO_GAPS/2+i]->re));
    printf(".\n");
    */  
