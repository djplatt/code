// amended 19/11/15 by DJP to compute FFTW plan once per q.
...
public:
    ...
    fftw_plan plan;
    bool fftw_plan_made;
    std::complex<double> *fftw_a;
    std::complex<double> *fftw_X;    
...
void DirichletGroup::DFTsum (complex<double> * out, complex<double> * in) {
	...
	fftw_a = new complex<double>[phi_q];
	fftw_X = new complex<double>[phi_q];

	plan = fftw_plan_dft(dft_dimension, dft_lengths,
			     (fftw_complex *)fftw_a,
			     (fftw_complex *)fftw_X,
    ...
    for(int n = 0; n < q; n++) {
        ...
        else fftw_a[dft_translation[n]] = in[n];
    }
    ...
    for(int n = 0; n < q; n++) {
        ...
        else out[n] = fftw_X[idft_translation[n]];
    }
}
