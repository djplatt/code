	       ...
	       bigint *squares, bigint *cubes, bigint *as, bigint *bs)
{
  ...
  // rhs is alway +ve, so get rid of -ve lhs
  while(lnodes[0].pq<0)
    {
      //print_node(lnodes);
      double dtarget_a=ceil(exp(log(-(double)lnodes[0].pq+1)*0.5))-1;
      if(dtarget_a>=w_H)
	{
  ...
  while(true)
      ...
      else
	{
	  if(right_duplicate)
	    {
	      if(rnodes[0].b<z_H-1) // more right nodes to add
		{
		  rnodes[0].b++;
		  rnodes[0].pq=bs[rnodes[0].a]+cubes[rnodes[0].b];
		  //printf("L Pushing ");print_node(lnodes[0]);
