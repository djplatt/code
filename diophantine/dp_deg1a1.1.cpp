  ...
  for(long int i=0;i<H;i++)
    {
      lnodes[i].a=0;
      lnodes[i].b=H-i-1;
      lnodes[i].pq=squares[0]-cubes[H-i-1];
    }
  int left_end=H-1;
  while(lnodes[0].pq<0)
    {
      //print_node(lnodes);
      long int target_a=ceil(exp(log((double)lnodes[0].b+1)*1.5))-1;
      //printf("b=%ld target a=%ld\n",lnodes[0].b,target_a);
      if(target_a<H)
	{
	  lnodes[0].a=target_a;
	  lnodes[0].pq=squares[target_a]-cubes[lnodes[0].b];
	}
      else
