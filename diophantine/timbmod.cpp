...
bool check_eqn(node *heap, bigint *xcubes, bigint *ycubes, bigint *zcubes, long int H, long int *solvec)
{
  ...
  while(true)
      ...
      while(z3<heap[0].p)
	{
      ...
      if(z3==heap[0].p)
	{
	  solvec[0]=heap[0].x;solvec[1]=heap[0].y;solvec[2]=z;
	  return(true);
	}
...
int main(int argc, char **argv)
{
  ...
  long int W,H,N,solvec[3];

  if(argc!=4)
    {
      printf("Incorrect command line, W H N. Exiting.\n");
      exit(0);
    }
  ...
  if(H<0)
    {
      printf("Bad H. Exiting.\n");
      exit(0);
    }

  printf("Searching for solutions to x^3+y^3=z^3+%ld in N^3 to height %lu\n",W,H*N);
  ...
  for(long int x1=0;x1<N;x1++)
    for(long int y1=0;y1<N;y1++)
      for(long int z1=0;z1<N;z1++)
	if((x1*x1*x1+y1*y1*y1-z1*z1*z1-W)%N==0)
	  {
	    //printf("Working modulo %lu with x=%lu y=%lu and z=%lu.\n",N,x1,y1,z1);
	    for(long int i=0,x=x1,y=y1,z=z1;i<H;i++)
	      {
		xcubes[i]=x;xcubes[i]*=x;xcubes[i]*=x;x+=N;
		ycubes[i]=y;ycubes[i]*=y;ycubes[i]*=y;y+=N;
		zcubes[i]=z;zcubes[i]*=z;zcubes[i]*=z;zcubes[i]+=W;z+=N;
	      }
	    if(check_eqn(nodes,xcubes,ycubes,zcubes,H,solvec))
	      {
		printf("solution found at x=%ld y=%ld z=%ld. Exiting.\n",solvec[0]*N+x1,solvec[1]*N+y1,solvec[2]*N+z1);
		exit(0);
	      }
	  }
}
