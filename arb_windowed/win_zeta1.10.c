...
//         1.9 Check for Newton Raphson wandering off into the ether.
//
...
void print_usage()
{
  printf("Usage:- win_zeta1.10 prec t_min num_its step outfile stat_pts?.\n");
...
if(atoi(argv[6]))
  ...
  for(it=0;it<num_its;it++,t0+=step)
      ...
      for(j=0,i=N/2-1;i>=N/2-int_step/2-INTER_SPACING*Ns;j++,i--)
	{
      ...
      for(i=0;i<100;i++)
	{
	  printf("t=%10.8e\n",t0+(double)i*one_over_A);
	  mpfi_c_print_str("",f_vec[N/2+i]);
	}
      */
