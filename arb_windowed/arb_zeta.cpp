...
#define T0_MIN (5000.0) // mimimal input t0
#define UPSAM (32) // the rate of upsampling from g to f
#define N ((int) 1<<22) // final FFT length
#define N1 ((int) N/UPSAM) // intermediate FFT length
#define one_over_A ((double) 21.0/4096.0) // output spacing
#define one_over_A1 ((double) one_over_A*UPSAM) // g/G spacing
#define h ((double) 420) // Gaussian width for f(t)
#define B ((double) N*one_over_A)

#define M ((int) 1700000)//2650000) // number of terms in F(x) sum
#define K ((int) 25)//10) // number of Taylor terms
#define gtwiderr_d ((double) 5.2e-50)//3.6e-60)
#define Gtwiderr_d ((double) 4.7e-113)//2.2e-162)
#define fhatsumerr_d ((double) 9.5e-241)//5.5e-34)
#define tayerr_d ((double) 5.6e-37)//2.8e-49)
#define fhattwiderr_d ((double) 1e-307) // actually smaller 
#define ftwiderr_d ((double) 4.8e-141)
#define Fmaxerr_d ((double) 2.9e-211) // Max mod of F(N1/(2B)) 
...
bool resolve_stat_point(arb_ptr tl, arb_ptr ftl, arb_ptr tm, arb_ptr ftm, arb_ptr tr, arb_ptr ftr, int left, sign_t this_sign, acb_t *f_vec, int64_t prec)
{
  ...
  while(true)
      ...
      if((sign(sp_ft)&this_sign)==0)
	{
      ...
      dir1=dir(ftl,sp_ft);
      dir2=dir(sp_ft,ftm);
      if((dir1==UNK)||(dir2==UNK))
	{
	  printf("Unknown direction in resolve_stat_point.\n");
	  printf("f(l)=");arb_printd(ftl,100);printf("\n");
	  printf("f(m)=");arb_printd(ftm,100);printf("\n");
	  printf("f(r)=");arb_printd(ftr,100);printf("\n");
	  printf("f(lm)=");arb_printd(sp_ft,100);printf("\n");
	  return(false);
	}
      ...
      if((dir3==UNK)||(dir4==UNK))
	{
	  ...
	  printf("f(r)=");arb_printd(ftr,100);printf("\n");
	  printf("f(lm)=");arb_printd(sp_ft,100);printf("\n");
	  printf("f(mr)=");arb_printd(sp_ft1,100);printf("\n");
	  return(false);
	}
      if(((this_sign==POS)&&(dir3==DOWN)&&(dir4==UP))||((this_sign==NEG)&&(dir3==UP)&&(dir4==DOWN)))
	{
      ...
      arb_set(ftr,sp_ft1);
      /*
      arb_printd(ftl,20);printf(" ");
      arb_printd(ftm,20);printf(" ");
      arb_printd(ftr,20);printf(" ");
      print_dir(dir1);printf(" ");
      print_dir(dir2);printf(" ");
      print_dir(dir3);printf(" ");
      print_dir(dir4);printf("\n");
      */
    }
}
...
double Nleft_int(long int t0_ptr, long int t1_ptr, acb_t *f_vec, double delta)
{
  long int res=0;
...
double Nright_int(long int t0_ptr, long int t1_ptr, acb_t *f_vec, double delta)
{
  long int res=0;
  long int ptr=t0_ptr;
  sign_t last_sign=sign(f_vec[ptr++]),this_sign;
...
long int turing_max(acb_t *f_vec, long int a_ptr, double a, long int b_ptr, double b, int64_t prec)
{
  ...
  arb_sub_d(tm_res,tm_res,Nright_int(a_ptr,b_ptr,f_vec,one_over_A),prec);
  ...
  printf("turing_max computed ");arb_printd(tm_tmp,10);printf("\n");
...
long int turing_min(acb_t *f_vec, long int a_ptr, double a, long int b_ptr, double b, int64_t prec)
{
  ...
  //printf("int S(t)=");mpfi_printn(tm_res,10);
  arb_sub_d(tm_res,tm_res,Nleft_int(a_ptr,b_ptr,f_vec,one_over_A),prec);
  //printf("int S(t) - int Nright");mpfi_printn(tm_res,10);
  ln_term(tm_tmp,tm_t0,tm_t1,tm_h,prec);
  //printf("int - t log(pi)/2=");mpfi_printn(tm_tmp,10);
  im_int(tm_tmp1,tm_t0,tm_t1,prec);
  ...
  printf("turing_min computed ");arb_printd(tm_res,10);printf("\n");
...
long int turing(acb_t *f_vec, long int a_ptr, double a, long int b_ptr, double b, long int last_max, int64_t prec)
{
  ...
  /*
  for(uint64_t ptr=a_ptr;ptr<=b_ptr;ptr++)
    {
      arb_printd(acb_realref(f_vec[ptr]),20);
      printf("\n");
    }
  */
  return(0);
...
int main(int argc, char **argv)
{
  ...
  exps1=(arb_t *) malloc((int_step/2+Ns*INTER_SPACING)*sizeof(arb_t));
  if(!exps1)
    {
