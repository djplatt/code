...
bool read_func(L_func_t *L, L_comp_t *Lc, L_family_t *Lf, FILE *infile, double normalisation, uint64_t prec)
{
  ...
  */
  ...
  // now divide each coeeficient by the normalisation
...
bool final_ifft(L_comp_t *Lc, L_func_t *Lfu, L_family_t *Lf, L_error_t *Le, uint64_t prec)
{
  ...
  */
  ...
  /*
  for(uint64_t i=0;i<Lc->N/2;i++)
    {
      printf("F_hat_twiddle(%lu) approx ",i);
      acb_printd(Lc->res[i],10);
      printf("\n");
    }
  */
...
void normalise(L_comp_t *Lc, L_family_t *Lf, uint64_t prec)
{
  ...
  */
  ...
  for(uint64_t n=Lc->N-1;n>Lc->N/2;n--,arb_add(t,t,one_over_A,prec))
    {
      arb_mul(tmp,pir4,t,prec);
      arb_exp(tmp1,tmp,prec);
      arb_mul(acb_realref(Lc->res[n]),acb_realref(Lc->res[n]),tmp1,prec);
...
int main(int argc, char **argv)
{
  ...
  while(fscanf(fnamefile,"%lu %lu %s\n",&wt,&L_func.N,fname)==3)
	...
	for(uint64_t n=0;n<L_comp.N;n++)
      ...
      */
      ...
      if(!do_pre_iFFT_errors(&L_comp,&L_family,&L_func,arb_prec))
	{
  ...
  for(int64_t n=L_comp.N/2+1,nn=n-L_comp.N;n<L_comp.N;n++,nn++)
    {
