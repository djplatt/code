...
typedef struct{
  ...
  double one_over_B;
  arb_t file_two_pi_by_B; // spacing of the Gs in the file
...
typedef struct{
  ...
  acb_t *ans[2]; // the coefficients a_n/sqrt(n)
  acb_t epsilon[2]; // the square root of the root number
...
typedef struct{
  ...
  arb_t A; // N/B
  uint64_t M; // truncation bound for sum 5-10 and Lemma 5.4
  ...
  acb_t **Gs; // the values of G^(k)(i/B)/k! read to be used
  int64_t *ms; // ms[m]=round(log(m/sqrt(N))*B)
  arb_t inv_B; // 1/B
  arb_t arb_two_pi_by_B;
  arb_t *ums; // ms[m]/B
  arb_t **sks; // (log(m/sqrt(N))-um)^k m=1..M, k=1..K
  acb_t *kres,*res[2],*skm;
