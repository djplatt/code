...
#define NEWTON_ITS (8) // Do 8 iterations with Newton Raphson to locate roots
...
mpfr_t znew_guess,zold_guess,ztn,zoffset,fl,fm,fr,tl,tm,tr;
...
// if fd_res is NULL, don't calc it
...
void resolve_stat_point(mpfr_ptr tl, mpfr_ptr ftl, mpfr_ptr tm, mpfr_ptr ftm, mpfr_ptr tr, mpfr_ptr ftr, int left, sign_t this_sign, mpfi_c_t *f_vec)
{
  ...
  while(TRUE)
    {
      mpfr_add(t,tl,tm,GMP_RNDN);
      mpfr_div_ui(t,t,2,GMP_RNDN);
      inter_t(ft,NULL,f_vec,t);
      if((mpfr_sign(ft)&this_sign)==0)
	{
	  //mpfr_printf("Sign change found at t=%.40Re\n",t);
	  mpfr_set(tr,tm,GMP_RNDN);
	  mpfr_set(ftr,ftm,GMP_RNDN);
	  mpfr_set(tm,t,GMP_RNDN);
	  mpfr_set(ftm,ft,GMP_RNDN);
	  return;
	}
      dir1=mpfr_dir(ftl,ft);
      dir2=mpfr_dir(ft,ftm);
      ...
      if((mpfr_sign(ft)&this_sign)==0)
	{
      ...
      dir1=mpfr_dir(ftm,ft);
      dir2=mpfr_dir(ft,ftr);
      if(((this_sign==POS)&&(dir1==DOWN)&&(dir2==UP))||((this_sign==NEG)&&(dir1==UP)&&(dir2==DOWN)))
	{
	  mpfr_set(tl,tm,GMP_RNDN);
	  mpfr_set(ftl,ftm,GMP_RNDN);
      ...
      printf("Process failed to converge.\n");
    }
}
...
{
  int i=start,count=0,last_pos=start;
  ...
  for(;i<=end;i++)
      ...
      if((this_sign&last_sign)==0) // valid sign change
	{
	  ...
	  if((res1_sign&res2_sign)!=0) // not a valid sign change
	    ...
	    {
	      count++;
	      mpfr_out_str(NULL,10,0,ztn,GMP_RNDN);
	      printf("\n");
	      if(count==20)
		exit(0);
	    }	  
