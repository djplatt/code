...
#define T0_MIN (50000.0) // mimimal input t0
...
#define intererr_d ((double) 0.0)//5.0e-41)
...
mpfr_t znew_guess,zold_guess,ztn,fl,fm,fr,tl,tm,tr;
...
void print_usage()
{
  printf("Usage:- win_zeta prec t0 t1 step outfile stat_pts?.\n");
  exit(1);
}
...
#define MAX_LNG_TERMS (12)
#define LNG_TERMS (MAX_LNG_TERMS)

mpfi_t lng_terms[MAX_LNG_TERMS],lng_err;

double ber_nums[MAX_LNG_TERMS]={1,-1,1,-1,5,-691,7,-3617,43867,-174611,854513,-236364091};
unsigned int ber_dens[MAX_LNG_TERMS]={6,30,42,30,66,2730,6,510,798,330,138,2730};
...
void init_lng()
{
  ...
  assert(T0_MIN-B/2.0>=45000);
  assert(LNG_TERMS==12);
  mpfi_set_d(lng_err,2.82e-102); // assumes 12 terms, t>=45000
...
void gamma_factor(mpfi_c_ptr res, mpfi_ptr gaussian, double t)
{
  ...
  // now do the sum of bernouli bits
...
void out_bytes(mpfr_ptr t, FILE *outfile)
{
  ...
  uint8_t byte1;
  //printf("Outputting ");
  //mpfr_out_str(NULL,10,0,t,GMP_RNDN);
  //mpfr_get_z(out_z,t,GMP_RNDN);
  bytes8=mpz_get_ui(out_z); // just grabs least sig. long uns int (8)
  fwrite(&bytes8,sizeof(uint64_t),1,outfile);
  //printf("\nLeast significant 8 bytes=%lX.\n",bytes8);
  mpz_fdiv_q_2exp(out_z,out_z,64);
  bytes8=mpz_get_ui(out_z);
  bytes4=bytes8&0xFFFFFFFF;
  fwrite(&bytes4,sizeof(uint32_t),1,outfile);
  //printf("Next least significant 4 bytes=%X.\n",bytes4);
  bytes8>>=32;
  if(bytes8>255)
    {
  ...
  byte1=bytes8;
  fwrite(&byte1,sizeof(uint8_t),1,outfile);
  //printf("Most significant byte = %X.\n",byte1);
  //if(++out_count>20)
  //exit(0);
}
...
void newton2 (mpfr_ptr new_guess, mpfr_ptr old_guess, mpfi_c_t *f_vec, mpfr_ptr tl, mpfr_ptr tr, mpfr_ptr fl, mpfr_ptr fr, int num_its)
{
  int i;
  /*
  mpfr_sub(new_guess,fr,fl,GMP_RNDN); // y0-y1
  mpfr_div(new_guess,fr,new_guess,GMP_RNDN); // y0/(y0-y1)
  mpfr_sub(old_guess,tr,tl,GMP_RNDN); // x1-x0
  mpfr_mul(new_guess,new_guess,old_guess,GMP_RNDN);
  mpfr_add(old_guess,new_guess,tl,GMP_RNDN);
  */
...
void resolve_stat_point(mpfr_ptr tl, mpfr_ptr ftl, mpfr_ptr tm, mpfr_ptr ftm, mpfr_ptr tr, mpfr_ptr ftr, int left, sign_t this_sign, mpfi_c_t *f_vec)
{
  dir_t dir1,dir2;
  //mpfr_t t,ft;
  ...
  while(TRUE)
      ...
      if((mpfr_sign(sp_ft)&this_sign)==0)
	{
      ...
      mpfr_add(sp_t,tm,tr,GMP_RNDN);
      mpfr_div_ui(sp_t,sp_t,2,GMP_RNDN);
      inter_t(sp_ft,NULL,f_vec,sp_t);
      if((mpfr_sign(sp_ft)&this_sign)==0)
	{
	  //mpfr_printf("Sign change found at t=%.40Re\n",sp_t);
      ...
      if(((this_sign==POS)&&(dir1==DOWN)&&(dir2==UP))||((this_sign==NEG)&&(dir1==UP)&&(dir2==DOWN)))
	{
      ...
      dir1=mpfr_dir(ftm,sp_ft);
      dir2=mpfr_dir(sp_ft,ftr);
      if(((this_sign==POS)&&(dir1==DOWN)&&(dir2==UP))||((this_sign==NEG)&&(dir1==UP)&&(dir2==DOWN)))
	{
	  mpfr_set(tl,tm,GMP_RNDN);
	  mpfr_set(ftl,ftm,GMP_RNDN);
...
{
  int i=start+1,count=0,last_pos=start;
	  ...
	  if(((this_dir==UP)&&(this_sign==POS))||((this_dir==DOWN)&&(this_sign==NEG)))
	    {
	      ...
	      resolve_stat_point(tl,fl,tm,fm,tr,fr,i-2,this_sign,f_vec);
		...
		{
		  printf("Not a valid sign change (left stat point).\n");
		  printf("right hand value =");mpfi_printn(ztmp,40);
		  mpfr_printf("At location %.40Re\n",zhigh);
		  mpfi_inter_t(ztmp,f_vec,zlow);
		  printf("left hand value  =");mpfi_printn(ztmp,40);
	      ...
	      if(mpfr_sgn(last_t)<=0)
		{
	      ...
	      if((res1_sign&res2_sign)!=0)
		{
		  printf("Not a valid sign change (right stat point).\n");
		  printf("right hand value =");mpfi_printn(ztmp,40);
		  mpfr_printf("At location %.40Re\n",zhigh);
		  mpfi_inter_t(ztmp,f_vec,zlow);
		  printf("left hand value  =");mpfi_printn(ztmp,40);
...
int turing(mpfi_c_t *f_vec, int a_ptr, double a, int b_ptr, double b, int last_max, FILE *outfile, bool stat_pts)
{
  // etimate maximum for N(b)

  uint8_t tmp[13];
  int i;
  for(i=0;i<13;tmp[i++]=0);
  ...
  if(last_max==0) // No previous run or previous run failed, so estimate min N(a)
    ...
    min_lo=last_max;
  int num_found,num_exp=max_hi-min_lo;
  fwrite(&a,sizeof(double),1,outfile);
  fwrite(&b,sizeof(double),1,outfile);
  fwrite(&num_exp,sizeof(int),1,outfile);
  if(stat_pts)
  ...
  if(stat_pts)
    {
      printf("Missed %d/%d zeros (running with stat points) in region %f to %f.\n",num_exp-num_found,num_exp,a,b);
      for(i=0;i<num_exp-num_found;i++)
	fwrite(tmp,sizeof(uint8_t),13,outfile);
      return(0);
    }
  ...
  printf("Will miss %d/%d zeros in region %f to %f even after %d stats.\n",num_exp-num_found-stats,num_exp,a,b,stats);
  for(i=0;i<num_exp-num_found;i++)
    fwrite(tmp,sizeof(uint8_t),13,outfile);
  return(0);
}
...
int main(int argc, char **argv)
{
  ...
  if(atoi(argv[6]))
    {
      printf("Running with stat points.\n");
      stat_pts=TRUE;
    }
  else
    {
  ...
  //main loop
  int num_its=(t1-t0)/step+1;
  printf("Aiming to do %d iteration(s) starting at t=%13.11e.\n",num_its,t0-step/2.0);
  fwrite(&num_its,sizeof(int),1,outfile);
  while(t0<=t1)
    {
