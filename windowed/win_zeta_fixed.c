...
long unsigned int zs_start,zs_end;
...
void print_usage()
{
  printf("Usage:- win_zeta_fixed prec t_min step outfile zs_start zs_end.\n");
  printf("finds zeros in [t_min,t_min+step].\n");
  printf("Looks for precisely zs_end-zs_start zeros.\n");
  exit(1);
}
...
long int turing(mpfi_c_t *f_vec, long int a_ptr, double a, long int b_ptr, double b, long int last_max, FILE *outfile, bool stat_pts)
{
  ...
  long int min_lo,max_hi=zs_end;//turing_max(f_vec,b_ptr,b,b_ptr+TURING_LEN,b+TURING_WIDTH);
  if(last_max==0) // No previous run or previous run failed, so estimate min N(a)
    min_lo=zs_start;//turing_min(f_vec,a_ptr-TURING_LEN,a-TURING_WIDTH,a_ptr,a);
