...
#define bool int
int HI_PREC;
#define HIGH_RATE (8*32)
#define LOG2_HIGH_RATE (8)
...
#define UPSAM (32*HIGH_RATE) // the rate of upsampling from g to f
#define N ((int) 1<<(20+LOG2_HIGH_RATE)) // final FFT length
#define N1 ((int) N/UPSAM) // intermediate FFT length
#define one_over_A ((double) 21.0/(4096.0*HIGH_RATE)) // output spacing
...
#define INTER_SPACING (5*HIGH_RATE) // Use every 5th sample
...
void print_usage()
{
  printf("Usage:- win_zeta_very_high prec t_min num_its step outfile stat_pts?.\n");
