...
#define UPSAM (256) // the rate of upsampling from g to f
#define N ((int) 1<<23) // final FFT length
#define N1 ((int) N/UPSAM) // intermediate FFT length
#define one_over_A ((double) 21.0/(4096.0*8)) // output spacing
...
#define N ((int) 1<<23) // final FFT length
#define N1 ((int) N/UPSAM) // intermediate FFT length
#define one_over_A ((double) 21.0/(4096.0*8.0)) // output spacing
#define one_over_A1 ((double) one_over_A*UPSAM) // g/G spacing
#define h ((double) 176431.0/2048.0) // Gaussian width for f(t)
#define B ((double) N*one_over_A)
...
#define INTER_SPACING (40) // Use every 5th sample
...
void print_usage()
{
  printf("Usage:- win_zeta_high prec t_min num_its step outfile stat_pts?.\n");
