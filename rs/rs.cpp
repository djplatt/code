...
int main(int argc, char ** argv)
{
  ...
  if(argc!=3)
    {
      printf("Usage:- rs <coeff file> <zeros file>\n");
      exit(0);
    }
  ...
  for(long int it=0;it<num_its;it++)
	...
	continue;
      ...
      srand(zs[0]);
      long int next_test=zs[0]+1+rand()%STEP;
      for(long int z=zs[0]+1;z<=zs[1];z++)
	{
#ifdef DEBUG
	  if(z==zs[0]+1000)
	    {
	      printf("Done 1000 zeros.\n");
	      exit(0);
	    }
#endif
	  ...
	  if(z!=next_test)
	    continue;
	  next_test+=rand()%STEP;
	  //std::cout << "Testing zero " << z << " at t=" << t << std::endl;
	  rl=rs(t-OP_ACC,coeffs);
#ifdef DEBUG
	  if((z%100)==0)
	    std::cout << "RS(" << t <<") returned " << rl << std::endl;
	  continue;
#endif
	  rr=rs(t+OP_ACC,coeffs);
	  if(((rl>=0)&&(rr>=0))||((rl<=0)&&(rr<=0)))
	    {
	      std::cout << "Problem with zero number " << z << " at " << t << std::endl;
	      std::cout << "Z either side returned " << rl << " and " << rr << std::endl;
	    }	  
	}
    }
  return(0);
