...
int main(int argc, char **argv)
{
  ...
  while(fread(&index,sizeof(uint64_t),1,infile)==1)
      ...
      if(conj_index==index) // a real character
	{
	  ...
	  printf("q %lu index %lu %lu sum 1/gamma = ",q,index,index);arb_printd(tot,20);printf("\n");
	  continue;
	}
      num_prims++;
      ...
      for(z=0;z<num_zeros1;z++)
	{
      ...
      printf("q %lu index %lu %lu sum 1/gamma = ",q,index,conj_index);arb_printd(tot,20);printf("\n");
