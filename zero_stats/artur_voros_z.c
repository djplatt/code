...
void do_gamma(arb_t res, arb_t gam, arb_t neg_s, int64_t prec)
{
  static int init=(1==0);
  ...
  arb_pow(res,tmp2,neg_s,prec); //(1/4+gam^2)^-s
}
...
int main(int argc, char ** argv)
{
  ...
  arb_t s;
  arb_init(s);
  arb_set_si(s,atol(argv[3]));
  arb_div_si(s,s,atol(argv[4]),prec);
