...
int main(int argc, char **argv)
{
  ...
  FILE *fname_file=fopen(argv[1],"r");
  if(!fname_file)
    {
  ...
  zero_t carry_over=zero_t(0,0,0);
  int_double last_t=0.0;
  char buff[1024];
  int_double smallest=100.0,largest=0.0;
  while(fscanf(fname_file,"%s\n",buff)==1)
    {
      FILE* zfile=fopen(buff,"rb");
      if(!zfile)
	{
      ...
      for(it=0;it<num_its;it++)
	{
	  ...
	  for(z=zs[0]+1;z<=zs[1];z++)
		...
		del_t=del_t+carry_over;
	      ...
	      if(last_t.left!=0.0)
		{
		  int_double rel_gap=(del_t+diff_error)*log(last_t/d_two_pi)/d_two_pi;
		  
		  if(-rel_gap.right < smallest.left)
		    {
		      printf("New smallest relative gap at zero %lu to %lu ending at ",z-1,z);
		      print_int_double(t);
		      printf(" relative gap ");
		    ...
		    {
		      if(-rel_gap.right < -smallest.right)
			{
			  printf("Overlapping relative gap at zero %lu to %lu ending at ",z-1,z);
			  print_int_double(t);
			  printf(" relative gap ");
			  print_int_double(rel_gap);
			  printf("\n");
		  ...
		  if(largest.left < -rel_gap.right)
		    {
		      printf("New largest relative gap at zero %lu to %lu ending at ",z-1,z);
		      print_int_double(t);
		      printf(" relative gap ");
		      print_int_double(rel_gap);
		      printf("\n");
