...
public:
  ...
  */
  ...
  friend zero_t operator += (zero_t &lhs, const zero_t &rhs);
};

inline zero_t operator += (zero_t &lhs, const zero_t &rhs)
{
__asm__ __volatile__(
		     ...
		     "adcq %5,%%r12\n\t"
		     "movq %%r12,%2\n\t"
		     :"=m" (lhs.a), "=m" (lhs.b), "=m" (lhs.c)
		     : "m" (rhs.a), "m" (rhs.b), "m" (rhs.c)
		     : "r12");      
 return(lhs);
}
...
int main(int argc, char **argv)
{
  ...
  for(it=0;it<num_its;it++)
      ...
      for(z=zs[0]+1;z<=zs[1];z++)
	{
	  ...
	  //printf("sum_zeros was %lu %lu %lu = ",sum_zeros.a,sum_zeros.b,sum_zeros.c);
	  //print_int_double_str("", (int_double) sum_zeros);
	  //printf("del_t=%lu %lu %lu\n",del_t.a,del_t.b,del_t.c);
	  sum_zeros+=del_t; // exact
	  //printf("sum_zeros now %lu %lu %lu = ",sum_zeros.a,sum_zeros.b,sum_zeros.c);
	  //print_int_double_str("", (int_double) sum_zeros);
	  t=(int_double) sum_zeros+zero_error;
	  //print_int_double_str("sigma del t=",t);
	  t+=st[0];
	  ...
	  if(!calced)
	      ...
	      if(diff.left<min_abs_smallest_gap)
		{
		  ...
		  printf("z=%lu last_t=",z);print_int_double(last_t);print_int_double_str(" t=",t);
		  printf("del_t=%lu %lu %lu =[ %20.18e , %20.18e ]\n",del_t.a,del_t.b,del_t.c,diff.left,-diff.right);
		  printf("sum_zeros=%lu %lu %lu\n",sum_zeros.a,sum_zeros.b,sum_zeros.c);
