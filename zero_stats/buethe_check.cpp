...
void R(int_double t, uint64_t N_t)
{
  int_double lnt=log(t)/2;
  int_double N=t/(2*d_pi)*(log(t/(2*d_pi*exp(int_double(1)))))+7.0/8.0;
  int_double diff1=abs(N-(N_t-1));
  int_double diff2=abs(N-N_t);
  if(-diff1.right>lnt.left)
    print_int_double_str("R violated at t=",t);
  if(-diff2.right>lnt.left)
    print_int_double_str("R violated at t=",t);
}
