...
void init_ws()
{
	uint64_t ws_ptr=1; // this is where n=4 starts
	uint64_t n,i;
	int_double two_pi_n;
...
void fft(int_complex *x,uint64_t n,int_complex *w)
{
	uint64_t i,j,k,l;
...
void ifft(int_complex *x,uint64_t n,int_complex *w) {
	uint64_t i,l=n>>1;
	int_complex ctemp;
...
uint64_t conv_sizes[MAX_DIMS],ws_ptr[MAX_DIMS];
...
void simple_dft(uint64_t n, uint64_t stride, int_complex *simple_vec)
{
	uint64_t n_2=n>>1,i,j,ind;
	int_complex w;
	if(!simple_dft_ws[n]) // haven't done one this big before
	{
		//printf("Preparing n=%ld.\n",n);
		if(!(simple_dft_ws[n]=(int_complex *) malloc(sizeof(int_complex)*n_2)))
		{
...
void convolve(int_complex *result,int_complex *f,int_complex *g,uint64_t n)
{
	uint64_t i;
	fft(f,n,&_ws[(n>>1)-1]);
	fft(g,n,&_ws[(n>>1)-1]);
...
void init_bluestein_fft(uint64_t n, uint64_t *conv_size,
						int_complex *b, int_complex *b_star_conj)
						/*
						n is an uint64_teger <=MAX_N
						...
						*/
{
	...
	for(i=1;i<n;i++)
	{
		ismod=(__int128_t)i*(__int128_t)i;
		ismod=ismod%(2*n);
		isqrmod=(uint64_t) ismod;
...
void bluestein_fft(uint64_t stride, int_complex *x, uint64_t n,
				   int_complex *a, int_complex *b,
				   int_complex *b_star_conj, uint64_t conv_size,
				   int_complex *b_spare)
				   /* 
...
bool read_factors(factor *factors, uint64_t n, ifstream &facs_file)
{
	uint64_t i,j;
	for(i=3;i<=n;i++)
	{
...
uint64_t fft_2_count=0;
inline void do_2_fft(uint64_t stride, int_complex *fft_vec)
{
...
uint64_t fft_4_count=0;
inline void do_4_fft(uint64_t stride, int_complex *fft_vec)
{
...
uint64_t fft_power_2_count=0;
inline void power_2_fft(uint64_t n, uint64_t stride, int_complex *fft_vec)
{
	uint64_t i,j;
	fft_power_2_count++;
	if(stride==1)
...
// don't call with n==0
inline uint64_t bin_power_2(uint64_t n)
{
	while((n&1)!=1)
		n>>=1;
	return(n==1);
...
void do_nd_fft(int_complex *fft_vec, uint64_t rank, const uint64_t *dims,uint64_t dist)
{
  // do a <rank>-d fft on <fft_vec> of length <length> where <dist>=prod(dims) apart
  uint64_t i=0,k,l,stride=dist,depth;
  /*
...
void fill_q_ns (uint64_t *q_n,// uint64_t *q_neg_n,
				uint64_t pr, uint64_t phi_q,
				uint64_t q)
{
	uint64_t i,pr_n;
	q_n[1]=0;
...
void fill_q_ns_2s(uint64_t *q_n, uint64_t q)
{
	uint64_t i,pr;
	pr=1;
	for(i=0;i<(q>>2);i++)
...
void make_offsets(uint64_t q, uint64_t *q_n, uint64_t *offset_vec, factor *factors,
				  uint64_t *a_n)
{
  uint64_t i,j,fac,fac1,offset,offset1,ptr;
...
bool non_prim(uint64_t a_n, uint64_t q, factor *factors)
{
	uint64_t i;
...
inline bool prim_p(uint64_t i, uint64_t p)
{
	return((i%p)!=0);
}
...
inline bool neg_one_p(uint64_t *coords, uint64_t n)
{
	uint64_t i,sum;
	sum=0;
	for(i=0;i<n;i++)
...
void prep_omegas(int_complex *omegas, uint64_t q, uint64_t phi_q, uint64_t *q_n,
				 int_complex *a, int_complex *b, int_complex *b_star_conj,
				 uint64_t conv_size, int_complex *b_spare)
{
  uint64_t n;
...
void prep_omegas_nd(uint64_t *offsets,uint64_t q, int_complex *omegas,uint64_t n_dims,uint64_t *dims,
					uint64_t phi_q)
{
  uint64_t n,n1,i,w_ptr=0;
  int_double theta=int_double(2.0)/q,q_minus_half;
...
uint64_t num_prims(uint64_t q,factor *factors) // returns number of primitive characters mod q
{
	uint64_t res=1,i,p;
	for(i=0;i<factors[q].num_facs;i++)
	{
