...
bool make_factors(factor *factors, const long unsigned int q_end)
{
  ...
  if(q_end<4)
    fatal_error("q_end must be at least 4.");
...
inline int_complex calc_r_n(long unsigned int r_n_terms, int_complex *s_array, int_double &delta,
      int_complex *r_n_vec)
{
		  ...
		  unsigned int q_end, FILE *out_file, qit_t *qits)
{
  ...
  for(q=q_start;q<=q_end;q++)
    ...
    q_s=q_it(q,s.imag.left,qits+q)*sqrt(int_double(q));
      ...
      if(co_prime(num,q))
	{
	  ...
	  out_val=taylor_err+conj(q_it(num,s.imag.left,qits+num))*q_s/sqrt(int_double(num));//pow(int_double(num),-0.5)*
