/*
File: int-l-func-zero5.7.cpp

Created: 9 November 2010 from int-l-func5.7.cpp
...
Implementation notes: Takes Output from l-func-mpfi-1.1 at s=1/2
            num_s (int)
            N (int)
...
{
  printf("Usage: int-l-func%s (q-start) (q-end) (ifname) (ofname) (N) (semaphore name) (qit file)\n",VERSION);
  ...
  printf("  (qit file)  - file containing qit data.\n");
  exit(1);
}
      ...
      for(i=0;i<phi_q;i++)
	{
	  ...
	  */
	  if(contains_zero(z.real))
	    printf("Can't tell if L(1/2,chi) vanishes q=%d index=%d.\n",q,i);
	  
	  print_int_double_str("L(1/2,chi)=",z.real);
          ...
          unsigned int q_end, FILE *out_file)
{
  ...
  for(q=q_start;q<=q_end;q++)
    ...
    q_s=q_it(q,s.imag.left)*sqrt(int_double(q));//pow(int_double(q),s.real.left);
      ...
      if(co_prime(num,q))
	{
	  ...
	  out_val=taylor_err+conj(q_it(num,s.imag.left))*q_s/sqrt(int_double(num));//pow(int_double(num),-0.5)*
...
int main(int argc, char **argv)
{
  ...
  if(!(qit_file=fopen(argv[8],"rb")))
    fatal_error("Error opening qit file. Exiting.\n");
  if(!read_qits(qit_file))
    fatal_error("Error reading qit_file. Exiting.\n");
  fclose(qit_file);
