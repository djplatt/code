/*
File: int-l-func5.1.cpp

Created: 10th Jan 2009

Version: <v> = 5.1

Last Modified: 22nd July 2009
      ...
      int_complex *r_n_vec,int_double &alpha)
{
	...
	for(i=0;i<r_n_terms;i++)
	  print_int_complex_str("r_n_vec[]= ",r_n_vec[i]);
      ...
      FILE *out_file)
{
  ...
  int_complex omega,omega_a,z,z1,q_minus_s,q_it_2;
  int dims[MAX_FACS];
  int no_dims;
...
//    printf("my_hur_ptr %ld\n",my_hur_ptr);
    while(s_done<num_s)
    {
      sin_cos(log(int_double(q))*(im_s_vec[s_done].im_s/2.0),
		  &q_it_2.imag,&q_it_2.real);
      q_minus_s=pow1(q,-0.5,-im_s_vec[s_done].im_s);
      for(i=1;i<q;i++)
        if(co_prime(i,q))
        {
          ...
          unsigned int q_end, FILE *in_file, FILE *out_file)
{
  ...
  int_double x,frac;
  int_complex q_minus_s,minus_s,q_pi_it_2;
  ...
  for(q=q_start;q<=q_end;q++)
	...
	print_int_complex_str("pow1= ",pow1(frac,-s.real.left,-s.imag.left));
        out_val=out_val+pow1(frac,-s.real.left,-s.imag.left);
	print_int_complex_str("s= ",s);
	printf("alpha = %10.8e\n",frac.left);
	print_int_complex_str("Zeta(s,alpha)= ",out_val);
