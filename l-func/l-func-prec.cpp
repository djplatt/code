...
Arbitary Precision Version of the Hurwitz Calculator.
...
typedef complex<float_precision> dcomplex;
typedef dcomplex int_complex;

typedef float_precision int_double;
...
void create_s_array(dcomplex &s,dcomplex *s_array, unsigned int n, int_double gap)
{
  ...
  s_array[0]=s*gap;
  for(i=1;i<n;i++)
      s_array[i*n+i]=dcomplex(real(s)+(int_double) (int) i,imag(s))*gap;
...
dcomplex zeta(dcomplex s,int_double alpha, unsigned int ZETA_TERMS)
{
	...
	for(i=1;i<=ZETA_TERMS;i++)
		tot+=pow((int_double)(int)i+(int_double) alpha,-s);
	return(tot);
}
		...
		unsigned int ZETA_TERMS)
{
  ...
  s_m=s+dcomplex((int)M,0);
  for(i=0;i<ZETA_TERMS;i++)
	  zeta_terms[i]=pow((int_double)1+(int_double)(int) i+(int_double) alpha,-s_m);
  ...
  for(i=M+1;i<n;i++)
  {
	  for(j=0;j<ZETA_TERMS;j++)
		  zeta_terms[j]/=((int_double)(int)j+(int_double)1.0+(int_double)alpha);
		  ...
		  dcomplex *r_n_vec)
{
  ...
  for(i=0;i<r_n_terms-1;)
    {
      term=s_array[i]*pow(delta,(int_double)(int) i+(int_double) 1);
      i++;
      term*=r_n_vec[i];
      res+=term;
    }
				  ...
				  unsigned int ZETA_TERMS)
{
  ...
  for(i=1;i<no_gaps;i++)
    make_r_ns(s_array,r_n_terms,M,&r_n_vals[i*r_n_terms],
	      &r_n_vals[(i-1)*r_n_terms],s,(int_double) 1.0-(int_double)(int)i*gap,zeta_terms,ZETA_TERMS);
  ...
  for(q=q_start;q<=q_end;q++)
  {
	  ...
	  q_minus_s=pow((int_double)(int)q,minus_s);
	  for(num=1;num<q;num++)
		  if(co_prime(num,q))  // q>num so this way round?
		  {
			  num_fracs++;
			  num_minus_s=pow((int_double) (int) num,minus_s);
			  frac=int_double((int)num)/(int_double) (int)q;
			  steps=ceil((int_double)(int)num/(gap*(int_double)(int)q));
...
int main(int argc, char **argv)
{
	...
	if(!out_file)
		fatal_error("Couldn't open ifile. Exiting.\n");
	gap=atof(argv[8]);
	if((gap<=(int_double)0.0)||(gap>=(int_double)1.0))
		print_usage();
	ZETA_TERMS=atoi(argv[9]);
	if(ZETA_TERMS<=0)
		print_usage();
	no_gaps=(int) ceil((int_double) 1.0/gap);
