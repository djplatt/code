...
Last Modified: 29th Jul16th September 2009
...
5.4 using semaphores to keep disk accesses apart
          ...
          unsigned int q_end, FILE *in_file, FILE *out_file)
{
  ...
  //sem_wait(lfuncsem); // wait for disk
  for(i=0;i<=no_gaps;i++)
  {
  ...
  //sem_post(lfuncsem); // let next process have it
  num_fracs=0;
...
int main(int argc, char **argv)
{
  ...
  */
  ...
  lfuncsem=sem_open("/lfuncsem",0);
  if(lfuncsem==SEM_FAILED)
    {
      perror("Error opening semaphore /lfuncsem.");
      exit(1);
    }
