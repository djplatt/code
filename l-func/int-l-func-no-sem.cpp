      ...
      for(i=0;i<phi_q;i++)
	{
      ...
      */
      for(i=1;i<phi_q;i++)  // i=0 corresponds to principal chi
	    ...
	    //if(i==1699){print_int_complex_str("finished omega[]= ",omegas[i]);}
  ...
  for(q=q_start;q<=q_end;q++)
      ...
      if(co_prime(num,q))
	{
	 ...
	 x=int_double(steps)-frac/gap;
	  ...
	  //printf("num=%d, csteps-dsteps=%30.28e, gap_ptr=%d\n",num,csteps-dsteps,gap_ptr);
	  ...
	  out_val+=pow(int_double(num),-0.5)*conj(q_it(num,s.imag.left))*q_s;
	      //printf("gap_ptr=%d\n",gap_ptr);
	      //print_r_n_vec(&r_n_vals[gap_ptr],r_n_terms);
	      //print_int_double_str("x=",x);
	      //print_int_double_str("frac=",frac);
	      //printf("H(%d/%d)=",num,q);print_int_complex_str("",out_val);
