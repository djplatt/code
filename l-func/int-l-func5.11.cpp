...
{
  printf("Usage: int-l-func%s (q) (ifname) (ofname) (N) (qit file)\n",VERSION);
  printf("  (q)         - integer >=3\n");
...
//
// Euler's totient of p_n where p_n=p^n
//
inline long unsigned int phi(const unsigned long int p, const unsigned long int p_n)
{
  return(p_n-p_n/p);
}

// compute a^b mod m
long unsigned int pow_mod(const long unsigned int a, const long unsigned int b, const long unsigned int m)
{
...
//
// Find a primitive root of i      
// will break if i does not have a pr
//
long unsigned int pr(const unsigned long int i, const factor *factors)
{
  long unsigned int phi=factors[i].phi;
  for(long unsigned int p=2;p<i;p++)
    {
      if(gcd(p,i)!=1)
	continue;
      bool good=true;
      for(long unsigned int j=0;j<factors[phi].num_facs;j++)
	{
...
//
// fill out the factors structure
//
// don't call with q_end<4
//
bool make_factors(factor *factors, const long unsigned int q_end)
{
  printf("Making factor database.\n");
  long unsigned int *primes,max_p=floor(sqrt(q_end));
  primes=(long unsigned int *) malloc(sizeof(long unsigned int)*(q_end+1));
  for(long unsigned int i=2;i<=q_end;i++)
    primes[i]=0;
  for(long unsigned int i=4;i<=q_end;i+=2)
    primes[i]=2;
  for(long unsigned int i=3;i<=max_p;i+=2)
    if(primes[i]==0)
      for(long unsigned int j=i*i;j<=q_end;j+=i)
	if(primes[j]==0)
	  primes[j]=i;
  ...
  for(long unsigned int f=5;f<=q_end;f++)
	...
	if(factors[n].primes[0]==primes[f])
	...
	else
	  {
	    ...
	    factors[f].primes[0]=primes[f];
	    for(long unsigned int i=1;i<factors[f].num_facs;i++)
	      {
  ...
  // now calculate phi(f)
  for(long unsigned int i=3;i<=q_end;i++)
    {
      factors[i].phi=1;
      for(long unsigned int j=0;j<factors[i].num_facs;j++)
  ...
  factors[3].pr=2;
  factors[4].pr=3;
  for(long unsigned int i=5;i<=q_end;i++)
    {
...
void create_s_array(const int_complex s,int_complex *s_array, const long unsigned int n, const double gap)
{
  long unsigned int i;
  ...
  for(i=1;i<n;i++)
    {
      ...
      //print_int_complex_str("s_array=",s_array[i]);
      s_array[i]/=i+1;
    }
  //for(i=0;i<n;i++)
  //print_int_complex_str("s_array=",s_array[i]);
...
inline int_complex calc_r_n(long unsigned int r_n_terms, int_complex *s_array, 
			    int_double &delta,int_complex *r_n_vec)
{
  long unsigned int i=r_n_terms-2;
...
double *lattice_buff;
long unsigned int lattice_buff_ptr=0;
...
{
  int_double ans;
  ans.set_left(lattice_buff[lattice_buff_ptr++]);
  ans.set_right(-lattice_buff[lattice_buff_ptr++]);
  return(ans);
}
...
{
  int_complex ans;
  ans.real.set_left(lattice_buff[lattice_buff_ptr++]);
  ans.real.set_right(-lattice_buff[lattice_buff_ptr++]);
  ans.imag.set_left(lattice_buff[lattice_buff_ptr++]);
  ans.imag.set_right(-lattice_buff[lattice_buff_ptr++]);
  return(ans);
}

inline void skip_int_complex(long unsigned int n)
{
  lattice_buff_ptr+=n*sizeof(int_complex);
}

//
// compute error from truncating Taylor series
//
int_complex taylor_error(int_complex &s, long unsigned int N,double gap,int_complex *s_array)
{
  int_double a,r,err;
  ...
  if(right(err)>=0.0)  // interval in (-infty,0.0]
    err.set_right(left(err));
  else
    {
      if(left(err)<=0.0) // interval in [0.0,infty)
	err.set_left(right(err));
      else
	{
	  if(right(err)>=left(err))
	    err.set_left(-right(err));
	  else
	    err.set_right(-left(err));
	}
...
long unsigned int hur_ptr=0,num_fracs;
int_complex *hur_vec;


inline int_complex calc_qit(long unsigned int n_dt, qit_t *qit)
{
...
// compute q^it
//
inline int_complex q_it(long unsigned int q, double t, qit_t *qit)
{
  double dn=t/(one_over_two_B/2.0);
  long unsigned int n=dn;
  if((dn-(double) n)!=0.0)
    fatal_error("q_it called with non-integral multiple of 2B.");
	    ...
	    qit_t *qits)
{
  ...
  for(i=1;i<q;i++)
    if(co_prime(i,q))
      a_n[j++]=i;
  if(pr)  // q has a primitive root so nice and easy 1-d DFT
  {
...
//    printf("my_hur_ptr %ld\n",my_hur_ptr);
    while(s_done<num_s)
    {
      q_comb=im_s_vec[s_done].pi_minus_it_2*q_minus_half*conj(/*n_it(q,im_s_vec[s_done].im_s/2.0));*/q_it(q,im_s_vec[s_done].im_s/2.0,qits+q));
      ...
      for(long unsigned int j=0;j<phi_q;j++)
	{
      ...
      */
	  ...
	  //printf("abs error in z=%d\n",abs_error(z.real));
          ...
          {
            printf("Imaginary part of z for character %u does not contain zero.\n",i);
	    print_int_complex_str("Fft_vec[]= ",Fft_vec[i]);
	    print_int_complex_str("z= ",z);
	    ...
	    print_int_complex_str("hur_vec[-1]=",hur_vec[my_hur_ptr-1]);
	    fatal_error("Exiting.");
          }
	  /*
...
//    printf("my_hur_ptr %ld\n",my_hur_ptr);
    while(s_done<num_s)
    {
      ...
      q_comb=im_s_vec[s_done].pi_minus_it_2*q_minus_half*conj(q_it(q,im_s_vec[s_done].im_s/2.0,qits+q));
      for(i=0;i<phi_q;i++)
		  ...
		  unsigned int q_end, FILE *out_file, qit_t *qits)
{
  ...
  for(q=q_start;q<=q_end;q++)
    ...
    q_s=q_it(q,left(s.imag),qits+q)*sqrt(int_double(q));
      ...
      if(co_prime(num,q))
	{
	  ...
	  out_val=taylor_err+conj(q_it(num,left(s.imag),qits+num))*q_s/sqrt(int_double(num));//pow(int_double(num),-0.5)*
	  ...
	  //print_int_complex_str("out_val=",out_val);exit(0);
	  //printf("out_val abs error = %d, %d.\n",abs_error(out_val.real),abs_error(out_val.imag));
	  //exit(0);
...
int main(int argc, char **argv)
{
  ...
  _fpu_rndd();
  //print_int_double_str("d_one=",d_one);
  //print_int_complex_str("c_one=",c_one);
  ...
  if((q<3)||(q>MAX_Q)||((q&3)==2))
    print_usage();
  if(q>MAX_Q)
    fatal_error("q exceeds MAX_Q.");
  in_file=fopen(argv[2],"rb");
  if(!in_file)
    {
      perror("Error opening in_file. ");
      printf("Failed to open file |%s|\n",argv[2]);
      fatal_error("Couldn't open in_file.");
    }


  if(!(factors=(factor *) calloc(q+1,sizeof(factor))))
    fatal_error("Error allocating memory for factors.");

  if(!make_factors(factors,q))
    fatal_error("Error reading factor file.");
  ...
  if(!qit_file)
    {
      ...
      printf("Failed to open file |%s|\n",argv[5]);
      fatal_error("Couldn't open qit file.");
    }

  qit_t *qits=(qit_t *) malloc(sizeof(qit_t)*MAX_Q+1);
  if(!qits)
    fatal_error("Error allocating memory for qits.");

  for(i=0;i<=MAX_Q;i++)
    if(fread(qits+i,sizeof(qit_t),1,qit_file)!=1)
      fatal_error("Problem reading qit file.");
  fclose(qit_file);
  ...
  */
  ...
  if((N>file_N)||(N<=0))
    fatal_error("N<=0 or exceeds N used to save lattice file.");
  fread(&rn,sizeof(int),1,in_file);
  if(rn!=1)
    fatal_error("RN must be 1.");
  out_file=fopen(argv[3],"wb");
  if(!out_file)
    fatal_error("Couldn't open out_file.");
  ...
  if(!(hur_vec=(int_complex *) _aligned_malloc(num_fracs*sizeof(int_complex)*num_s,16)))
    fatal_error("Failed to allocate memory for hur_vec.");
  ...
  if(!(lattice_buff=(double *) malloc((13+4*(no_gaps+1)*file_N)*num_s*sizeof(double))))
    fatal_error("Failed to allocate memory for lattice_buff.");
  ...
  */
  for(i=0;i<num_s;i++)
    {
      ...
      if(!num_fracs)
	fatal_error("Error running Hurwitz routine.");      
    }
  ...
  if(!(omegas=(int_complex *) _aligned_malloc((q-1)*sizeof(int_complex),16)))
    fatal_error("Error allocating memory for omegas.");

  if(!(q_n=(unsigned int *) calloc(q-1,sizeof(unsigned int))))
    fatal_error("Error allocating memory for q_n.");

  if(!(a_n=(unsigned int *) calloc(q-1,sizeof(unsigned int))))
    fatal_error("Error allocating memory for a_n.");

  if(!(offset_vec=(unsigned int *) calloc(q-1,sizeof(unsigned int))))
    fatal_error("Error allocating memory for offset_vec.");
