      ...
      FILE *out_file)
{
  ...
  int_complex omega,omega_a,z,z1,q_minus_s,q_it_2,q_comb;
  int dims[MAX_FACS];
  int no_dims;
...
//    printf("my_hur_ptr %ld\n",my_hur_ptr);
    while(s_done<num_s)
    {
      sin_cos(log(int_double(q)/d_pi)*(im_s_vec[s_done].im_s/2.0),
		  &q_it_2.imag,&q_it_2.real);
      q_minus_s=pow1(q,-0.5,-im_s_vec[s_done].im_s);
      q_comb=q_minus_s*q_it_2;
      for(i=1;i<q;i++)
        if(co_prime(i,q))
        {
...
//          fread(&fft_vec[q_n[i]],sizeof(int_complex),1,hur_file);
	  ...
	  z=z*q_comb;
	    ...
	    print_int_complex_str("(q/pi)^(it/2) ",q_it_2);
	    print_int_complex_str("q^(-s) ",q_minus_s);
	    /*	    if(neg_one)
