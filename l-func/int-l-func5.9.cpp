...
int_complex q_it(double t)
{
  int_double tlnq=lnq*t;
  int_complex res;
  sin_cos(tlnq,&res.imag,&res.real);
  return(res);
}

int_complex n_it(unsigned long int n, double t)
{
  int_double tlnn=log(int_double(n))*t;
  int_complex res;
  sin_cos(tlnn,&res.imag,&res.real);
  return(res);
}
      ...
      FILE *out_file)
{
  ...
  if(pr)  // q has a primitive root so nice and easy 1-d FFT
  {
    ...
    prep_omegas(omegas,q,phi_q,q_n,a,bs,b_star_conjs,conv_sizes[0]);
    //for(i=0;i<phi_q;i++) print_int_complex_str("prepped omegas[]= ",omegas[i]);
    s_done=0;
    my_hur_ptr1=hur_ptr;
    my_hur_ptr=hur_ptr;
//    printf("my_hur_ptr %ld\n",my_hur_ptr);
    while(s_done<num_s)
    {
      q_comb=im_s_vec[s_done].pi_minus_it_2*q_minus_half*conj(q_it(im_s_vec[s_done].im_s/2.0));
      for(i=1;i<q;i++)
        if(co_prime(i,q))
	  Fft_vec[q_n[i]]=hur_vec[my_hur_ptr++];
      bluestein_fft(1,Fft_vec,phi_q,a,bs,b_star_conjs,conv_sizes[0]);
      ...
      for(i=0;i<phi_q;i++)
	{
	  ...
	  for(j=0;j<factors[q].num_facs;j++)
	      ...
	      if(!contains_zero(z.imag))
		{
		  ...
		  print_int_complex_str("q^(it)=",q_it(im_s_vec[s_done].im_s));
		  print_int_complex_str("q^(it/2)=",q_it(im_s_vec[s_done].im_s/2.0));
		  print_int_complex_str("q_comb=",q_comb);
		  if(neg_one)
          ...
          unsigned int q_end, FILE *out_file)
{
  ...
  for(q=q_start;q<=q_end;q++)
    ...
    //lnq=ln_q(q);
    //assert(s.real.left==0.5);
    q_s=q_it(s.imag.left)*sqrt(int_double(q));//pow(int_double(q),s.real.left);
      ...
      if(co_prime(num,q))
	{
	  ...
	  out_val=taylor_err+conj(n_it(num,s.imag.left))*q_s/sqrt(int_double(num));//pow(int_double(num),-0.5)*
...
int main(int argc, char **argv)
{
  ...
  if((q<3)||(q>MAX_Q)||((q&3)==2))
    print_usage();
  if(q<<1>=MAX_CONV)
    fatal_error("q exceeds that supported by MAX_CONV (see fft_defs.h). Exiting.\n");
  ...
  for(i=0;i<num_s;i++)
    {
      ...
      num_fracs=calc_hurwitz1(s,N,file_N,gap,s_array,r_n_vals,no_gaps,q,q,out_file);
