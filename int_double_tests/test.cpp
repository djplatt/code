...
bool testc (unsigned int n)
{
  ...
  for(type1=POS;type1<=CROSS_ZERO;type1++)
      ...
      for(i=0;i<n;i++)
	{
	  ...
	  y=rand_double(type2);
	  int_double y2=sqr(y);
	  if(!contains_zero(x*x-y2-(x+y)*(x-y)))
	    {
	      printf("Test number %d.\n",i);
	      my_print("x",x);
	      printf("y %20.18e\n",y);
  ...
  for(int i=0;i<n;i++)
    {
      ...
      if(!contains_zero(sx*sx-x))
	{
	  printf("Test number %d.\n",i);
	  my_print("x",x);
	  my_print("sqrt(x)",sx);
	  my_print("sqrt(x)^2-x",sx*sx-x);
	  return(false);
	}
...
bool test3(unsigned int n)
{
  ...
  unsigned int i;
  char type1;
  for(x.set_left(0.0);left(x)<M_PI/2;x.set_left(left(x)+1.0/(double) n))
    {
      x.set_right(-left(x)-0.00000001);
      ...
      if(!contains_zero(sqr(sin_x)+sqr(cos_x)-1))
	{
	  ...
	  printf("\n");
	  print_int_double_str("sin^2+cos^2-1=",sqr(sin_x)+sqr(cos_x)-1);
	  return(false);
	}
    }
  return(true);
