...
err3A(x,lam,T1)=
{
   return(2*N_high(T1)*GA(x,lam,T1,1/2));
}
 ...
 */
   ...
   */
   ...
   lam=solve(lam=2^(-100),2^(-10),all_errsA(x,lam,T1)-0.248);
/*
   print("********************fixing lam***********************");
...
*/
   ...
   sieve_err=0.25;
   x0=floor(solve(x0=x/2,x,x0*(exp(calc_tau(x0,lam,0.25)))-x));
   print("initial x0=",x0);
/*   if(evenp(x0),x0=x0+1);
...
*/
   ...
   x0=x-asw/2+1;
   print("final x0=",x0);
   print("final sieve start =",x-asw+1);
   ...
   sieve_err=solve(err=1e-20,0.25,sieve_width(x0,lam,err)-asw);
