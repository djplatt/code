/* Pari code to support version 27 */
R=5.573412;
...
k0(sig,lx)=1.0/(exp(((10-16*sig)/3)*sqrt(lx/R))*(sqrt(lx/R))^(5-2*sig));

C3(sig,lx,k)=2*exp(-2*sqrt(lx/R))*lx^2*k;

C4(sig,lx,k)=exp(lx*(sig-1))*(2/Pi*lx/R+1.8642)*k;

C5(sig,lx,k)=8.01*C2(sig)*exp(-2*sqrt(lx/R))*lx/R*k;

C6(sig,lx)={local(k);k=k0(sig,lx);return(2.0025*2^(5-2*sig)*C1(sig)+C3(sig,lx,k)+C4(sig,lx,k)+C5(sig,lx,k))};

e0(sig,lx)=C6(sig,lx)*exp((10-16*sig)/3*sqrt(lx/R))*(lx/R)^((5-2*sig)/2);

sig=0.98;
for(n=1,4,printf("$%d\\,000$ & $%3.2f$ &  $%5.2f$ & $%.3f$ & $%.3f$ & $%.3e$\\\\\n",n,sig,C6(sig,n*1000),(5-2*sig)/2,-(10-16*sig)/3,e0(sig,n*1000)))
sig=0.99;
for(n=5,10,printf("$%d\\,000$ & $%3.2f$ &  $%5.2f$ & $%.3f$ & $%.3f$ & $%.3e$\\\\\n",n,sig,C6(sig,n*1000),(5-2*sig)/2,-(10-16*sig)/3,e0(sig,n*1000)))
