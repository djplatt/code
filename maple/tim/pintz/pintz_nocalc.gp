...
N(sig,T)=2.2*T^(8*(1-sig)/3)*log(T)^(5-2*sig)+6*log(T)^2;
...
t3a(lx,T,del,H)={local(this_e,best_e,K,lam);K=1;lam=(T/H);best_e=t3c(lx,T,del,H,K,lam);while(true,K++;lam=(T/H)^(1/K);this_e=t3c(lx,T,del,H,K,lam);if(this_e>best_e,KK=K-1;llam=(T/H)^(1/KK);print("t3a returning ",best_e," ",llam);return(best_e),best_e=this_e))};

err(lx,T,del,H)={local(tt1,tt2,tt3);tt1=t1(lx,T);tt2=t2(lx,T,del,H);tt3=t3a(lx,T,del,H);print("err returning ",tt1+tt2+tt3);return(tt1+tt2+tt3)};
...
We then iterate over T=10^16..10^43, delta=0.007..0.028 to find a sweet spot.
*/
chk(llx)={local(lx,e0,T,lT,del,T0,del0,e1lT0,best_K,best_lam);lx=log(exp(llx)-1);e0=1;T=1e16;lT=16;while(T<1e44,del=0.007;while(del<0.029,e1=err(lx,T,del,H);if(e1<e0,e0=e1;T0=T;lT0=lT;del0=del;best_K=KK;best_lam=llam);del+=0.001);T*=10;lT++);printf("$%d$ & $10^{%d}$ & %.3f & %d & %4.3f & %5.4E \\\\\n",lx,lT0,del0,best_K,best_lam,myroundup(e0,4));return(myroundup(e0,4));};
