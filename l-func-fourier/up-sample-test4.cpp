...
#define IN_POINTS (615)

#define SKIP_POINTS (POINTS_IN_FILE-IN_POINTS-(IN_POINTS-1)*SPACING)

// up sample by a factor of
#define UP_SAMPLE (10)

#define OUT_POINTS (UP_SAMPLE*IN_POINTS*2)
...
int main(int argc, char** argv)
{
	...
	p1 = fftw_plan_dft_1d(IN_POINTS*2, reinterpret_cast<fftw_complex*>(signal),
	...
	for(i=0;i<IN_POINTS;i++)
	{
		...
		signal[IN_POINTS*2-i-1]=signal[i];
		for(k=0;k<SPACING;k++)
		{
			fread(&left,sizeof(double),1,infile);
			fread(&right,sizeof(double),1,infile);
...
//	signal[IN_POINTS-1]=(signal[0]+signal[IN_POINTS-2])/2.0;
	...
	for(i=0;i<IN_POINTS*2;i++)
		signal[i]/=(double)(IN_POINTS*2);
...
// put in the +ve frequencies
	for(j=0;j<IN_POINTS;j++)
		result[i++]=signal[j];
// zero pad
	for(k=0;k<OUT_POINTS-IN_POINTS*2;k++)
		result[i++]=c_zero;
// put in the -ve frequencies
	for(;j<IN_POINTS*2;j++)
		result[i++]=signal[j];
...
if(choice==3)
printf("%d zeros found in output.\n",count_zeros(result,OUT_POINTS/2));
