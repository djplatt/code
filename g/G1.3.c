...
/* assumes that stored values are more accurate than can be represented */
...
int load_zeros()
{
    ...
    {
	mpfr_inp_str(rho,zero_file,10,GMP_RNDD);
	mpfr_set(rho1,rho,GMP_RNDU);
      mpfr_nextabove(rho1);
      mpfi_init(zeros[i]);
	mpfi_interv_fr(zeros[i],rho,rho1);
