...
Build instructions: gcc -oG1.3 G1.3.c -O2 -lmpfi -lmpfr -lgmp
...
#define ZEROS_FILE "zeros4"
#define ZEROS_FILE_PREC (100)
/* can't work to better than this -1 bits */
...
int load_zeros()
{
    ...
    fscanf(zero_file,"%Lu",&NUM_ZEROS);
    printf("Going to read %Lu zeros.\n",NUM_ZEROS);
    ...
    mpfr_init2(rho,ZEROS_FILE_PREC);

    for(i=0;i<NUM_ZEROS;i++)
    {
	mpfr_inp_str(rho,zero_file,10,GMP_RNDN);
	mpfi_init(zeros[i]); /* less precise than rho */
	mpfi_set_fr(zeros[i],rho);
    };
...
void print_usage()
{
	printf("Usage: G <bits_prec> <x> <lambda power> <taylor_n> <h_max> <G(1) height>\n%lu <= bits_prec <= %lu\n",
	       MPFR_PREC_MIN,ZEROS_FILE_PREC-1);
	printf("x >= 2\n");
	printf("lambda power >= 0\n");
...
/*  check all the command line arguments are ok, if not print message
    ...
    prec=atoi(argv[1]);
    if((prec<MPFR_PREC_MIN)||(prec>ZEROS_FILE_PREC-1))
    {
	print_usage();
	return(QUIT_CODE);
    };
...
*/
    ...
    mpfr_set_ui(lambda,1,GMP_RNDN);
    for(i=0;i<lambda_n;i++)
        mpfr_div_ui(lambda,lambda,2,GMP_RNDN);    
