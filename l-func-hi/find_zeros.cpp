...
{
  //mpfi_mul_d(res,t0_ptr,one_over_A);printf("t0=");mpfi_printn(res,300);
...
void out_bytes(mpfr_ptr t, FILE *outfile)
{
  ...
  if(mpfr_cmp_ui(out_tmp,0)<=0)
    {
      printf("Error consecutive zeros were the same or decreasing. Exiting.\n");
      exit(0);
    }
...
void bracket_t(mpfi_ptr res1, mpfi_ptr res2, mpfr_ptr tn, mpfr_ptr t_now,double one_over_A)
{
  scale_t(tn,t_now,one_over_A);
  //assert(is_exact(
  mpfi_set_fr(res1,tn);//));
  mpfi_add_d(res2,res1,0.5);
...
void do_zero(mpfr_ptr lt, mpfr_ptr lf, mpfr_ptr rt, mpfr_ptr rf, mpfi_t *lam, mpfi_t *lam_bar, double one_over_A,FILE *outfile)
{
  ...
  bracket_t(dz_res1,dz_res2,dz_tmp1,dz_tmp2,one_over_A);
  /*
...
uint64_t proper_count(mpfi_t *lam, mpfi_t *lam_bar, uint64_t end, double one_over_A, FILE *outfile, uint64_t q)
{
  // start the difference counter from 0, or 8 if q=3
  if(q==3)
    {
      mpfr_set_ui(last_t,8,GMP_RNDN);
      mpfr_mul_2ui(last_t,last_t,101,GMP_RNDN);
    }
  else
    mpfr_set_ui(last_t,0,GMP_RNDN);
...
void find_zeros_real(mpfi_t *lam, uint64_t num_s, double del_t, bool even_p, uint64_t q, FILE *outfile,uint64_t index)
{
  ...
  if(qc==turing_mx) // all zeros accounted for
    {
      //printf("Writing %lu zeros\n",qc);
      fwrite(&qc,sizeof(uint64_t),1,outfile);
      uint64_t pc=proper_count(lam,lam,a,del_t,outfile,q);
      if(pc!=qc)
	{
...
void find_zeros_cmplx(mpfi_t *lam, mpfi_t *lam_bar, uint64_t num_s, double del_t,bool even_p, uint64_t q, FILE *outfile, uint64_t index)
{
  uint64_t a=T0/del_t,b=T1/del_t;
  uint64_t turing_mx=turing_max(lam,lam_bar,a,T0,b,T1,even_p,del_t,q)+
    turing_max(lam_bar,lam,a,T0,b,T1,even_p,del_t,q);
  printf("Turing max=%lu\n",turing_mx);
  ...
  uint64_t qc=quick_count_cmplx(lam,lam_bar,a);
  ...
  if(qc==turing_mx)
    {
      //printf("Writing %lu zeros\n",qc);
      fwrite(&qc,sizeof(uint64_t),1,outfile);
      uint64_t pc=proper_count(lam,lam_bar,a,del_t,outfile,q);
      //printf("pc=%lu\n",pc);
      write_null_record(outfile);
      pc+=proper_count(lam_bar,lam,a,del_t,outfile,q);
      //printf("pc=%lu\n",pc);
      if(pc!=qc)
	{
	  printf("Index=%lu pc=%lu qc=%lu\n",index,pc,qc);
...
int main(int argc, char **argv)
{
  if(argc!=5)
    {
      printf("Usage:- %s <specfile> <this_proc> <num procs> <outfile>.\n",argv[0]);
      exit(0);
    }
  ...
  uint64_t index,index_counter=0,real_index_counter=0;
	  ...
	  if((index_counter%num_procs)==this_proc)
	    {
	      ...
	      find_zeros_real(lam,num_s,del_t,even_p,q,outfile,index);
	    }
	  real_index_counter++;
	  index_counter++;
	}
	  ...
	  if((index_counter%num_procs)==this_proc)
	    {
	      ...
	      fwrite(&index,sizeof(uint64_t),1,outfile);
	      find_zeros_cmplx(lam,lam_bar,num_s,del_t,even_p,q,outfile,index);
	    }
	  index_counter++;
	}
    }
  printf("%lu complex character pairs read and %lu real characters.\n",index_counter-real_index_counter,real_index_counter);
  return(0);
}
