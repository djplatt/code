...
// arb_zeta_once.cpp
//
// do arb_zeta on one interval
// don't use length M arrays anywhere.
//
...
{
  printf("Usage:- %s prec t_min step.\n",cmd);
  printf("Verifies RH in [t_min,t_min+step].\n");
  exit(1);
}
...
int main(int argc, char **argv)
{
  ...
  this_time=time(NULL);
  printf("Time to initialise = %G seconds.\n",difftime(this_time,last_time));
  last_time=this_time;
