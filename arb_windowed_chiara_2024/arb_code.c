...
arb_t big_st,little_st;
int64_t big_zero,little_zero;
...
void St(arb_t gamma, int64_t z, int64_t prec)
{
  ...
  /*
  //printf("S(t) near zero %ld ranges ",z);
  //arb_printd(rtmp3,20);
  //printf("\n");
  */
}
...
int main(int argc, char**argv)
{
  ...
  int64_t st=istart_zero,missed=0;
  printf("First zero processed was %ld\n",st);
  for(int64_t it=0;it<num_its;it++)
    {
      fmpz_set_si(start_zero,st);
      int64_t found=acb_dirichlet_platt_hardy_z_zeros((arb_ptr) zeros,start_zero,n_zeros,prec);
      if(found==0) // total loss
	{
	  printf("didn't find any zeros starting from %ld. Exiting.\n",st);
	  return 0;
	}
      missed+=n_zeros-found;	  
      for(int64_t z=0;z<found;z++)
  ...
  if(missed==0)
    printf("All zeros found.\n");
  else
    printf("missed %ld zeros.\n",missed);
