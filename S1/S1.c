...
void do_s1(arb_t *zeros, int64_t prec)
{
  arb_t eps; // step size
  arb_init(eps);
  arb_set_d(eps,1.0/256.0); // small enough for T_MAX=530
  ...
  while(1==1)
	...
	break;
      ...
      arb_add_ui(tmp1,tmp4,6,prec);
      arb_sub(tmp2,tmp1,tmp3,prec);
      arb_sub(tmp1,tmp2,th_int,prec);
      arb_abs(tmp1,tmp1);
      arb_sub(tmp2,tmp1,ab,prec); // must be negative

      if(!arb_is_negative(tmp2))
	{
	  printf("Test failed at T+eps = ");arb_printd(T_eps,10);printf("\n");
	  exit(0);
	}
      
      arb_set(T,T_eps); // T<-T+eps
      arb_add(th_int,th_int,th_eps,prec); // th_int <-1/pi int_6^{T} theta(t) dt
      arb_set(N1,N1_eps); // int_0^T N(t) dt
    }
  printf("Finished with T = ");arb_printd(T,10);
  printf("\n1/Pi Int_6^T theta(t) dt = ");arb_printd(th_int,10);
  printf("\nN1(T) = ");arb_printd(N1,10);printf("\n");
  return;
}
