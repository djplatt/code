...
#define S168_PI // subtract S1(168 pi) before check
...
void do_s1(arb_t *zeros, int64_t prec)
{
  ...
  arb_t S168_pi;
  arb_init(S168_pi);
  arb_set_d(S168_pi,-986);
  arb_set_ui(eps,1);
  arb_add_error(S168_pi,eps);
  arb_div_ui(S168_pi,S168_pi,1000,prec);
  printf("S(168 pi) in ");arb_printd(S168_pi,20);printf("\n");
#endif
  ...
  while(1==1)
    {
      // if T = limit, leave town
      if(arb_equal_si(T,T_MAX))
	break;
      arb_add(T_eps,T,eps,prec);
      do_th_eps(th_eps,T,T_eps,prec); // compute th_eps=1/pi int_T^T+eps theta(t) dt
      n=do_N1_eps(N1_eps,N1,T_eps,zeros,eps,n,prec); // compute N1_eps=N1(T+eps)
                                                     // sets n to N(T+eps)
      arb_union(tmp1,N1,N1_eps,prec); // [N1(T),N1(T+eps)]
      arb_union(tmp2,T,T_eps,prec); // [T,T+eps]
      a_bit(ab,tmp2,prec); // A0 + A1 log [T,T+eps]
      arb_union(tmp3,zero,th_eps,prec); // [0,1/pi int_T^{T+eps} theta(t) dt]

      arb_sub(tmp4,tmp1,tmp2,prec);
      arb_sub(tmp1,tmp4,tmp3,prec);
      arb_sub(tmp4,tmp1,th_int,prec);
#ifdef S168_PI
      arb_sub(tmp4,tmp4,S168_pi,prec);
#endif
      arb_abs(tmp4,tmp4);
      arb_sub(tmp3,tmp4,ab,prec); // must be negative

      if(!arb_is_negative(tmp3))
	{
	  printf("Test failed at T+eps = ");arb_printd(tmp2,10);printf("\n");
	  /*
	  if(!first_fail)
	    exit(0);
	  */
      ...
      arb_set(T,T_eps); // T<-T+eps
      arb_add(th_int,th_int,th_eps,prec); // th_int <-1/pi int_0^{T} theta(t) dt
      arb_set(N1,N1_eps); // int_0^T N(t) dt
    }
  printf("Finished with T = ");arb_printd(T,10);
  printf("\n1/Pi Int_0^T theta(t) dt = ");arb_printd(th_int,10);
  printf("\nN1(T) = ");arb_printd(N1,10);
  arb_sub(tmp1,N1,T,prec);
