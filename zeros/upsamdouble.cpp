...
void make_chis(unsigned int q, unsigned int index, unsigned int pr, unsigned int phi, unsigned int no_dims, int_complex *chis)
{
  ...
  for(n=1;n<phi/2;n++)
	...
	w-=phi;
      ...
      //printf("%d ",a);print_int_complex_str("",chis[a]);
      if(even_p)
	chis[q-a]=chis[a];
      else
	chis[q-a]=-chis[a];
...
inline int conj_index(int index, unsigned int q, factor *factors)
{
  ...
  long int res;
  /*
  for(int i=0;i<factors[q].num_facs;i++)
    printf("%lu\n",factors[q].facs[i]);
  */
  make_coords(coords,q,index,factors);
  //printf("int=%d\n",sizeof(int));
  //printf("Coords[0] was %d\n",coords[0]);
  ...
  //printf("Coords[0] now %d\n",coords[0]);
  for(int i=1;i<factors[q].num_facs;i++)
    {
      //printf("Coords[%d] was %d\n",i,coords[i]); 
      //printf("phi=%lu\n",factors[factors[q].facs[i]].phi);
      coords[i]=factors[factors[q].facs[i]].phi-coords[i];
      //printf("Coords[%d] now %d\n",i,coords[i]);
    }
  res=coords[0];
  for(int i=1;i<factors[q].num_facs;i++)
    {
...
int main(int argc, char **argv)
{
  ...
  while(fread(&qs,sizeof(q_state),1,infile))
	...
	{
	  facs_p=true;
	  factors=(factor *)malloc(sizeof(factor)*(qs.q+1));
	  if(!factors)
	    {
