...
#define INDEX (55546)
#define RATE (8)
#define OFFSET_START (97)
#define OFFSET_END (100)
...
{ 
  ...
  //print_int_complex_str("res=",res);
  //print_int_complex_str("q^(-s)=",pow(q,minus_s));
  res*=pow(q,minus_s);
  return(res);
}

int_complex make_omega(const int_complex &z, const int_complex &old_omega)
{
  print_int_complex_str("Making Omega for ",z);
  if(contains_zero(z.real))
    {
      printf("Using pre-calculated omega.\n");
      return(old_omega);
    }
  int_double arg_z=-argument(z);
  int_complex res;
  if(arg_z.left<d_pi_2.left)
    arg_z+=d_pi;
  else
    {
      if(arg_z.right<d_pi_2.right)
	arg_z-=d_pi;
    }
  sin_cos(arg_z,&res.imag,&res.real);
  print_int_complex_str("New Omega is ",res);
  return(old_omega);
}
  ...
  while(fread(&qs,sizeof(q_state),1,infile))
    {
      ...
      if(qs.index!=INDEX)
	continue;
      //index=conj_index(qs.index,qs.q,factors);

      qs.offset1=OFFSET_START;
      qs.offset2=OFFSET_END;
      print_qs(qs);
      printf("rate=%d\n",qs.rate);
      omega=int_complex(int_double(qs.omega[0],qs.omega[1]),int_double(qs.omega[2],qs.omega[3]));
      for(im_s=qs.gap*(qs.n0+qs.offset1/(double) qs.rate);im_s<=qs.gap*(qs.n02+qs.offset2/(double) qs.rate);im_s+=qs.gap/(double) (qs.rate*RATE))
	{
	  printf("Trying t =%30.28e\n",im_s);
	  hur_init(s_array,0.5,im_s);
	  res=lambda(0.5, im_s, qs.q, qs.index, factors, omega, qs.neg_one,s_array);
	  print_int_double_str("Lambda t=",res.real);
	  ...
	  /*
	  mid_sign=sign(res.real);
	  if(mid_sign==qs.exp_sign)
	    {
	      printf("q=%d index=%d im_s=%30.28e Type=%d Success.\n",qs.q,qs.index,im_s,qs.type);
	      if(qs.exp_sign==POS)
		printf("Expecting +ve.\n");
	      else
		printf("Expecting -ve.\n");
	      print_int_double_str("F(t)=",res.real);
	      break;
	    }
	  */
	}
