...
// upsamstatpt.cpp
//
// tries to resolve stat point problems where
// upsampling has remained one side of zero
...
int main(int argc, char **argv)
{
  ...
  if(argc!=4)
    {
      printf("usage:- upsamstatpt <infile> <factors file> <outfile>\nExiting.\n");
      exit(1);
    }
  ...
  while(fread(&qs,sizeof(q_state),1,infile))
	...
	{
	  printf("Non stationary point problem. Skipping.\n");
	  fwrite(&qs,sizeof(q_state),1,outfile);
	  continue;
	}
      else
	{
	  t2=qs.n0*qs.gap+qs.offset1*qs.gap/(double) qs.rate;
	  t1=t2-1.0/qs.rate;
	  t3=t2+1.0/qs.rate;
	}
      hur_init(s_array,0.5,t1);
      x1=lambda(0.5, t1, qs.q, qs.index, factors, omega, qs.neg_one,s_array);
      print_int_complex_str("x1=",x1);
      hur_init(s_array,0.5,t2);
      x2=lambda(0.5, t2, qs.q, qs.index, factors, omega, qs.neg_one,s_array);
      print_int_complex_str("x2=",x2);
      hur_init(s_array,0.5,t3);
      x3=lambda(0.5, t3, qs.q, qs.index, factors, omega, qs.neg_one,s_array);
      print_int_complex_str("x3=",x3);
    }
  fclose(infile);
  facs_file.close();
  fclose(outfile);
