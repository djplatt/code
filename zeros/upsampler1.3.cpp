...
#define one_over_two_B (5.0/64.0) // aka gap
#define INTER_N (20)
...
inline int num_zeros2 (int b, int_double *re_zs,im_s *im_s_vec) 
{
  int count=0,ptr=UP_SAMPLE_SACRIFICE;
  ...
  while(ptr<=b-2)
      ...
      if((this_sign==POS)&&(y1>y2)&&(y3>y2)) // min above zero
	{
	  ...
	  stat_count++;
	  count+=up_num_zeros(&out_array[UP_SAMPLE_SACRIFICE*UP_SAMPLE_RATE],im_s_vec,POS,POS,
			      2,ptr,re_zs,re_zs);
	  ptr++;
	  continue;
	}
      if((this_sign==NEG)&&(y2>y1)&&(y2>y3)) // max below zero
	{
	  up_sample_load(&re_zs[ptr-UP_SAMPLE_SACRIFICE]);
	  stat_count++;
	  count+=up_num_zeros(&out_array[UP_SAMPLE_SACRIFICE*UP_SAMPLE_RATE],im_s_vec,NEG,NEG,
			      2,ptr,re_zs,re_zs);
	  ptr++;
	  continue;
	}
...
inline int num_zeros(int a, int b, int_double *re_zs, im_s *im_s_vec)
{
  return(num_zeros1(a,b,re_zs)+num_zeros2(b,re_zs,im_s_vec));
}
   ...
   while(fread(&q,sizeof(int),1,infiles[0]))
       ...
       for(prim=0;prim<num_chi;prim++)
	 {
	   ...
	   if(prim<=prim1)
		 ...
		 {
		   cz1=num_zeros(0,turing_start,re_zs1,im_s_vec);
		   no_zeros+=cz1;
	       ...
	       else
		 {
		   ...
		   cz1=num_zeros(0,turing_start,re_zs1,im_s_vec);
		   cz2=num_zeros(0,turing_start,re_zs2,im_s_vec);
		   no_zeros+=cz1+cz2;
		   diff1=check_zeros(cz1,turing_start,turing_end,q,(neg_one ? gamma_a : gamma),
				     arg_omega,re_zs1,im_s_vec,index,t_sam_start);
