...
dir_t dir(int_double &x1, int_double &x2)
{
  int_double diff=x2-x1;
  if(diff.right>0.0)
    return(DOWN);
  if(diff.left>0.0)
    return(UP);
...
long unsigned int n_zeros(int_double *re_zs, long unsigned int start, long unsigned int end)
{
  long unsigned int ptr=start,n_zeros=0;
  bool zero_passed=true;
  sign_t last_sign=sign(re_zs[ptr]),this_sign;
  dir_t last_dir=UNK,this_dir;
  while(ptr<end)
    {
      ptr++;
      
      //print_int_double(re_zs[ptr]);
      this_sign=sign(re_zs[ptr]);
      if(this_sign==UNK) this_sign=last_sign;
      this_dir=dir(re_zs[ptr-1],re_zs[ptr]);
      //printf(" signs ");print_sign(last_sign);print_sign(this_sign);printf(" dirs ");
      //print_sign(last_dir);print_sign(this_dir);printf("\n");

      if((this_sign+last_sign)==1)
	{
	  n_zeros++;
	  zero_passed=true;
	  last_dir=UNK;
      ...
      if((this_dir+last_dir)==1)
	{
	  if(!zero_passed)
	    {
	      
	      printf("False stat point found.\n");
	      //print_int_double_str("",re_zs[ptr-2]);
	      //print_int_double_str("",re_zs[ptr-1]);
	      //print_int_double_str("",re_zs[ptr]);
...
int main(int argc, char **argv)
{
  ...
  int diff1,cz1,cz2,no_zeros=0,nz;
  ...
  while(fread(&q,sizeof(int),1,infiles[0]))
      ...
      if(q>MAX_Q)
	{
      ...
      //print_int_double_str("d_inter_err=",d_inter_err);exit(0);
      ...
      for(prim=0;prim<num_chi;prim++)
	{
	  ...
	  if(prim<=prim1)
	      ...
	      if(prim==prim1) // it's a real character
		{
		  ...
		  cz1=n_zeros(re_zs1,0,turing_start);
		  //rig_num_zeros_first(0,turing_starta,re_zs1,re_zs1);
	      ...
	      else
		{
		  ...
		  cz1=n_zeros(re_zs1,0,turing_starta);
		  //rig_num_zeros_first(0,turing_starta,re_zs1,re_zs2);
		  cz2=n_zeros(re_zs2,0,turing_starta);
		  //rig_num_zeros_first(0,turing_starta,re_zs2,re_zs1);
