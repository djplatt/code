...
bool make_factors(factor *factors, int q_end)
{
  ...
  for(int i=199990;i<200000;i++)
    {
      printf("%d has %d factors, phi(%d)=%d, pr(%d)=%d, factors are",i,factors[i].num_facs,i,factors[i].phi,i,factors[i].pr);
      for(int j=0;j<factors[i].num_facs;j++)
	printf(" %d %d",factors[i].primes[j],factors[i].facs[j]);
      printf("\n");
    }
  exit(0);
  */
...
int_complex chis[MAX_Q],*sincoss;

void init_sincos(int q, factor* factors)
{
  int phi_q;
  if((factors[q].primes[0]==2)&&(factors[q].facs[0]>=8))
    phi_q=factors[q].phi/4;
  else
    phi_q=factors[q].phi/2;
  //printf("Initialising %d sin cos values.\n",phi_q*2);
  sincoss=(int_complex *)malloc(sizeof(int_complex)*phi_q*2);
  if(!sincoss)
    {
      printf("Error allocating memory for sin cos vector. Exiting.\n");
      exit(0);
    }
  sincoss[0]=c_one;
  for(int i=1,ptr=1;i<phi_q*2;ptr++)
    if(gcd(ptr,q)==1)
      {
	sin_cospi(int_double(ptr)/phi_q,&sincoss[i].imag,&sincoss[i].real);
	i++;
      }
}

int_complex mysin_cospi(int ptr, bool neg_p)
{
  //printf("In mysin_cospi with ptr=%u\n",ptr);
  //print_int_complex_str("computed chi=",z);
  int_complex res=sincoss[ptr];
  if(neg_p)
    res.imag=-res.imag;
  //print_int_complex_str("my chi      =",res);
  //print_int_complex_str("diff=",res);
  return(res);
}
...
void make_chis1(unsigned int q, unsigned int index, unsigned int pr, unsigned int phi, int_complex *chis)
{
  ...
  bool neg_p=false; // do we do sincos(-theta)?
  ...
  if (q==16) neg_p=!neg_p;//four_pi_over_phi=-four_pi_over_phi;
  chi_ptr=5;
  chis[1]=c_one;
  if(even_p)
    chis[q-1]=c_one;
  ...
  for(pow=1;pow<phi_by_2;pow++)
	...
	w-=phi_by_2;
      ...
      chis[chi_ptr]=mysin_cospi(w,neg_p);
      if(even_p)
...
void make_chis(unsigned int q, unsigned int index, unsigned int pr, unsigned int phi, unsigned int no_dims, int_complex *chis)
{
  ...
  a=pr;
  bool neg_p=true;
  //two_pi_over_phi=int_double(-2.0)/phi;//-d_two_pi/phi;
  ...
  if(no_dims==1)
	...
	{neg_p=!neg_p;//two_pi_over_phi=-two_pi_over_phi;
	}
    }
  else
    {
	  ...
	  if(power_2_p(phi))
	    {
	      //printf("phi=%d\n",phi);
	      //	  if((ln2(phi)&1)==0)
	      neg_p=!neg_p;//two_pi_over_phi=-two_pi_over_phi; // phi=16,64,256 ...
	    }
	  else
	    {
  ...
  for(n=1;n<phi/2;n++)
	...
	w-=phi;
      ...
      //sin_cospi(theta,&chis[a].imag,&chis[a].real);
      //int_double diff=mod(chis[a]-mysin_cospi(w,neg_p));
      //if(diff.left>1e-10)
      //{
      //printf("w=%d a=%d\n",w,a);
      //print_int_complex_str("original ",chis[a]);
      //print_int_complex_str("mine     ",mysin_cospi(w,neg_p));
      //}
      chis[a]=mysin_cospi(w,neg_p);
      if(even_p)
	chis[q-a]=chis[a];
      else
	chis[q-a]=-chis[a];
...
{ 
  ...
  for(n=1;n<q;n++)
	...
	{	
	  ...
	  //printf("Zeta(s,%d/%d)*chi(%d)=",n,q,n);print_int_complex_str("",hur);
...
inline int_complex lambda(double re_z, double im_z, int q, int index, factor *factors, const int_complex &omega, int neg_one, int_complex *s_array)
{
  ...
  //print_int_complex_str("L returned ",res); exit(0);
...
int main(int argc, char **argv)
{
  ...
  if(argc!=3)
    {
      printf("usage:- upsamdouble <infile> <outfile>\nExiting.\n");
      exit(1);
    }
  ...
  if(fread(&qs,sizeof(q_state),1,infile)!=1)
    {
      printf("Failed to read from infile %s. Exiting.\n",argv[1]);
      exit(0);
    }
  //printf("q=%u\n",qs.q);
  factors=(factor *)malloc(sizeof(factor)*qs.q);
  if(!factors)
    {
      printf("Failed to allocate memory for factors. Exiting.\n");
      exit(1);
    }
  make_factors(factors,qs.q);
  int this_q=qs.q;
  init_sincos(this_q,factors);
  rewind(infile);
  ...
  while(fread(&qs,sizeof(q_state),1,infile))
    {
      if(this_q!=qs.q)
	{
	  printf("Error. Upsamdouble only handles a single q. Exiting.\n");
	  exit(0);
	}
