...
#define MAXN    (8)   /* max FFT size was2^21 */
...
{
  mpfi_out_str(stdout,10,3,c.re);
  mpfi_out_str(stdout,10,3,c.im);
};
...
void initfft() 
{
	...
	complex_init(w[1]);
	mpfi_const_pi(temp);
	mpfi_div_ui(temp,temp,3); // mod 7 so w^6=1
	...
	mpfi_set_ui(w[3].re,0);     // only actually got 6
	mpfi_set_ui(w[3].im,0);
	/*	
...
int main(int argc,char **argv) 
{
  ...
  mpfr_inp_str(z,ifile,10,GMP_RNDN);
  mpfi_set_fr(fft_vec[0].re,z);
  mpfi_set_ui(fft_vec[0].im,0);
  mpfr_inp_str(z,ifile,10,GMP_RNDN);
  mpfi_set_fr(fft_vec[2].re,z);
  mpfi_set_ui(fft_vec[2].im,0);
  mpfr_inp_str(z,ifile,10,GMP_RNDN);
  mpfi_set_fr(fft_vec[1].re,z);
  mpfi_set_ui(fft_vec[1].im,0);
  mpfr_inp_str(z,ifile,10,GMP_RNDN);
  mpfi_set_fr(fft_vec[4].re,z);
  mpfi_set_ui(fft_vec[4].im,0);
  mpfr_inp_str(z,ifile,10,GMP_RNDN);
  mpfi_set_fr(fft_vec[5].re,z);
  mpfi_set_ui(fft_vec[5].im,0);
  mpfr_inp_str(z,ifile,10,GMP_RNDN);
  mpfi_set_fr(fft_vec[3].re,z);
  mpfi_set_ui(fft_vec[3].im,0);
  mpfi_set_ui(fft_vec[6].re,0);
  mpfi_set_ui(fft_vec[6].im,0);
  mpfi_set_ui(fft_vec[7].re,0);
  mpfi_set_ui(fft_vec[7].im,0);
