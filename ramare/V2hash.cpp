...
compute V_2^# as per Ramare's PsiTilde-6-1 paper
define b(chi)=L'(0,chi)/L(0,chi) for all odd primitive characters 
             =lim s->0 L'(0,chi)/L(0,chi)-1/s for all even primitive characters
             = log(2Pi) when q=1
then V_2^#(q)=sum over prim characters |b(chi_+log(q/2Pi)-gamma|^2

do for modulus from q-start to q-end inclusive.
Divide result by phi_2(q) (number of primitive characters)
	...
	if(prim_p(i,factors[q].primes[0]))//&&(conj_j(k,n_prims,q)>=k)) // only do primitive conjugates
	  {
	    if(i&1) // odd
	      {
		res=Fft_copy1[i]/Fft_copy[i]-d_ln_two_pi-d_gamma+log_q;
		//odd_sum+=norm(res);
	      }
	    else // even
	      {
		...
		res=Fft_vec[i]/Fft_copy1[i]-d_ln_two_pi-d_gamma+log_q;
		//even_sum+=norm(res);
	      }
	    all_sum+=norm(res);
	    k++; // count primitive character
	  }
      ...
      for(i=0,k=0;i<phi_q;i++)
	{
	  ...
	  if(primitive)//&&(conj_j(k,n_prims,q)>=k))
	      ...
	      else // even
		{
	      ...
	      all_sum+=norm(res);
	      k++; // count the primitive character
	    }
