...
#define FFT_SACRIFICE (FFT_IN_LEN/4)
...
void up_sample(unsigned long int start, int_double *re_zs1, int_double *re_zs2)
{
 ...
 int main(int argc, char **argv)
 {
   ...
   if(argc!=3)
     {
       printf("Usage:- upsam <infile> <outfile>.\n");
   ...
   while(fread(&q,sizeof(int),1,infile))
       ...
       if(real_p) // it's a real character
	 {
	   ...
	   printf("Looking for %d zeros. ",diff);
	   printf("Sign changes alone account for %lu zeros.\n",simple_count(re_zs1,turing_starta));
	 }
       else // it's a complex character
	 {
	   ...
	   printf("Looking for %d zeros ",diff);
	   printf("Sign changes alone account for %lu zeros.\n",simple_count(re_zs1,turing_starta)+simple_count(re_zs2,turing_starta));
	 }
     }
 }
