...
/* copies the M entries of the bit arrays src, isqf to dest, sqf
   m/M times */
{
  unsigned int i, j, offb, ib0;

  memset(dest,0,(m-1)/8+1);

  memcpy(dest,src,(M-1)/8+1);

  for(i=M; i<m; i+=M) {
    offb = i%(8*sizeof(ulong));
    ib0 = i/(8*sizeof(ulong));
    
    if(offb) {
      for(j=0; j<(M-1)/(8*sizeof(ulong)); j++) {
	dest[ib0+j]   |= src[j]<<offb;	
	dest[ib0+j+1] |= src[j]>>((8*sizeof(ulong))-offb);
      }

      dest[ib0+j] |= src[j]<<offb;
      if(ib0+j+1<=(m-1)/(8*sizeof(ulong))) 
	dest[ib0+j+1] |= src[j]>>((8*sizeof(ulong))-offb);
    } else  
      memcpy(dest+ib0,src,(M-1)/8+1);
  }
}
