...
main(int argc, char *argv[])
{
  ...
  if(v==1) {
    printf("Let m_1(x) = \\check{m}(x) - 1,\n");
    printf("    m_2(x) = \\check{\\check{m}}(x) - 2\\log x + 2 \\gamma.\n");
  } else {
    printf("Let m_1(x) = \\check{m}(x) - 2,\n");
    printf("    m_2(x) = \\check{\\check{m}}(x) - 4\\log(x/2) + 4 \\gamma.\n");
  }
 
  printf("For x\\leq %ld\n",N);
  for(j=1; j<=2; j++)
    printf("\t|m_%d(x)|\\leq %g/x + %.10g/sqrt(x)\n",j,-bar[j].right,
       -sqrt((int_double) maxp[j]).right);
  for(j=0; j<=2; j++)
    printf("For %ld\\leq x\\leq %ld:\t |m_%d(x)|\\leq %.10g/sqrt(x) \n",
       ns[j],N,j,
