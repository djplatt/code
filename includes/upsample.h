...
int_double up_n_twiddle( int a,  int b, int_double *re_zs, im_s *im_s_vec, int t_sam_start)
{
  ...
  int_double end_point=int_double(b*one_over_two_B),res=int_double(0.0);
  ...
  while(j<=b)
      ...
      if(n!=0)
	{
	  ...
	  res+=(end_point-int_double(i*one_over_two_B,j*one_over_two_B))*n;
	  n1+=n;
	}
...
// assume unk in TZ are sign changes
int_double rig_n_twiddle2 (int t1, int t2, int_double *re_zs, im_s *im_s_vec, FILE *outfile, q_state &qs)
{
  double delta=(one_over_two_B)/(double) UP_SAMPLE_RATE,endpt=t2*one_over_two_B;
  double last_change=t1*one_over_two_B,this_change;
  int_double res=d_zero;
  int ptr=t1,offset=1;
  sign_t last_sign=sign(re_zs[t1]),this_sign;
  bool unk_p=false; int unk_ptr,unk_offset;
	  ...
	  if(unk_p)
	    {
	      ...
	      qs.offset1=unk_offset;qs.offset2=offset-1;
	      printf("Sign strayed into unknown and back in TZ near n0=%d-%d offset=%d-%d.",unk_ptr,ptr,unk_offset,offset-1);
	      printf("Assuming pair of zeros here.\n");
	      // here unk_offset points to first unk
	      // offset points to last unk + 2
	      // integral goes from unk_offset-1 to offset-1
	      res+=int_double(endpt-ptr*one_over_two_B-(offset-1)*delta,endpt-unk_ptr*one_over_two_B-(unk_offset-1)*delta)*2.0;
	      if(this_sign==POS)
		qs.exp_sign=NEG;
	      else
		qs.exp_sign=POS;
      ...
      if(this_sign==UNK)
	{
      ...
      unk_p=false;
      this_change=ptr*one_over_two_B+(offset-1)*delta;
      //printf("Zero found at ptr=%d between offset=%d and offset=%d\n",ptr,offset-2,offset-1);
      res+=calc_int(last_change,this_change,endpt);
      last_change=this_change;
      //print_int_double_str("res is now=",res);
...
// assume unk regions are sign changes
int rig_calc_zeros_cmplx2( int t1_ptr, int t2_ptr, int q,
			  int_double &gamma_int, int_double *re_zs1, int_double *re_zs2, im_s *im_s_vec,
			   int_complex &omega2,
			   int index, FILE *outfile, q_state &qs)
{
  ...
  double t1=t1_ptr*one_over_two_B;//im_s_vec[t1_ptr].im_s;
  double t2=t2_ptr*one_over_two_B;//im_s_vec[t2_ptr].im_s;
  int_double ntwiddle1=rig_n_twiddle2(t1_ptr,t2_ptr,re_zs1,im_s_vec,outfile,qs);
  qs_omega(qs,omega2);
  int_double ntwiddle2=rig_n_twiddle2(t1_ptr,t2_ptr,re_zs2,im_s_vec,outfile,qs);
				...
				int index)
{
  ...
  //printf("Turing zone is from %g to %g\n",t1,t2);
  ...
  calc_zeros=(lnt+gamma_int*2/(t2-t1))/d_pi-(ntwiddle1+ntwiddle2)/(t2-t1)+s_t/(t2-t1); // Turing's estimate
  //print_int_double_str("Turing's Estimate = ",calc_zeros); exit(0);
...
// assume unk in TZ are sign changes
int rig_calc_zeros_re2( int t1_ptr, int t2_ptr, int q,
				int_double &gamma_int, int_double &arg_omega,int_double *re_zs, im_s *im_s_vec,
			int index, FILE *outfile, q_state &qs)
{
  //printf("Reached rig_calc_zeros.\n");
  ...
  double t1=t1_ptr*one_over_two_B;//im_s_vec[t1_ptr].im_s;
  double t2=t2_ptr*one_over_two_B;//im_s_vec[t2_ptr].im_s;
  int_double ntwiddle=rig_n_twiddle2(t1_ptr,t2_ptr,re_zs,im_s_vec,outfile,qs);
