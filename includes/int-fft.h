...
int_complex _ws[MAX_CONV],a[MAX_CONV],bs[MAX_CONV],b_spare[MAX_CONV],b_star_conjs[MAX_CONV];
						...
						*/
{
	...
	if(conv_size[0]>MAX_CONV)
	  {printf("MAX_CONV exceeded in init_bluestein_fft. Exiting.");
	    exit(0);}
	pi_over_n=d_one/n;
	b[0]=c_one;
	b_star_conj[0]=c_one;
				   ...
				   */
{
	...
	for(i=0;i<conv_size;i++)
		b_spare[i]=b[i];
	//for(i=0;i<conv_size;i++) print_int_complex_str("a",a[i]);
	//for(i=0;i<conv_size;i++) print_int_complex_str("b",b_spare[i]);
	convolve(a,a,b_spare,conv_size); // a<- a*b
	//for(i=0;i<conv_size;i++) print_int_complex_str("a*b",a[i]);
	x[0]=a[0];
	for(i=1,j=stride;i<n;i++,j+=stride)
  ...
  for(i=0;i<rank;i++)
	...
	{	
	  ...
	  for(k=0;k<depth*(stride<<1);k+=(stride<<1))
	    for(l=0;l<stride;l++)
	      {
		//printf("Doing length 2 FFT starting at %lu with stride %lu\n",k+l,stride);
      ...
      if(dims[i]==4)
	{
	  ...
	  for(k=0;k<depth*(stride<<2);k+=(stride<<2))
	    for(l=0;l<stride;l++)
	      {
		//printf("Doing length 4 FFT starting at %lu with stride %lu\n",k+l,stride);
      ...
      if((dims[i]>MAX_SIMPLE_DFT)&&(bin_power_2(dims[i])))
	{
	  ...
	  for(k=0;k<depth*dims[i]*stride;k+=dims[i]*stride)
	    for(l=0;l<stride;l++)
	      {
		//printf("Doing length %lu FFT starting at %lu with stride %lu\n",dims[i],k+l,stride);
				 ...
				 unsigned int conv_size, int_complex *b_spare)
{
  ...
  //for(n=0;n<phi_q;n++)
  //print_int_complex_str("pre FFT omega=",omegas[n]);
  bluestein_fft(1,omegas,phi_q,a,b,b_star_conj,conv_size,b_spare);
  //for(n=0;n<phi_q;n++)
  //print_int_complex_str("post FFT omega=",omegas[n]);
  //exit(0);
  for(n=0;n<phi_q;n++)
