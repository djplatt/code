...
void acb_one(acb_t res, const acb_t x, int64_t prec)
{
  acb_set_ui(res,1);
}
...
void arb_maxd(arb_ptr res, 
	      ...
	      uint64_t steps)
{
  arb_maxd(res,f1,acb_one,low,hi,prec,steps);
}
...
void comp_error(arb_t err, double maxd, int64_t n, int64_t prec)
{
...
void arb_one (arb_t res, const arb_t x, int64_t prec)
{
  arb_set_ui(res,1);
}
...
void molin_int(arb_ptr res, int64_t n, 
	       ...
	       int64_t prec)
{
  molin_int(res,n,f1,arb_one,maxd,low,hi,prec);
}
