...
{
  printf("Usage:- %s (spec_file) (file_stub) (n-procs) (this-proc)\n",str);
  printf("spec_file - contains number of files followed by filename one per line.\n");
#ifdef EVEN
  printf("file_stub - has _even_q_n.dat appended for ouput\n");
#else
  printf("file_stub - has _odd_q_n.dat appended for ouput\n");
#endif
...
{
  if(one_over_A>=2*M_PI)
    fatal_error("Must have 1/2A<2Pi."); 
...
#else
  sprintf(buff,"%s_odd_%d_%d.dat",stub,q,index);
#endif
...
void save_data(unsigned int q,unsigned int index,const int_complex &omega,bool neg_one,bool real_p,unsigned int N0,FILE *outfile)
{
  ...
  my_fwrite(&index, sizeof(unsigned int),1,outfile);
  my_fwrite(&omega, sizeof(int_complex),1,outfile);
  my_fwrite(&neg_one, sizeof(bool),1,outfile);
  my_fwrite(&real_p, sizeof(bool),1,outfile);
  my_fwrite(&N0, sizeof(unsigned int),1,outfile);
  for(int n=0;n<N0;n++)
    my_fwrite(&Fft_Vec[n].real, sizeof(int_double),1,outfile);
}
  ...
  for(int n=1;n<num_files;n++)
    {
      ...
      if(!contains_zero(i-val))
	{
	  printf("int_complex mismatch.\n");
	  print_int_complex_str("read ",i);
	  print_int_complex_str("vs   ",val);
	  printf("Exiting.\n");
	  exit(0);
	} 
    }
	      ...
	      unsigned int n_p, unsigned int t_p)
{
  ...
  F_hat_t_o_err=F_hat_twiddle_err(delta_o,logdelta_o,logq);
  print_int_complex_str("F_hat_twiddle_err=",F_hat_t_o_err);
  for(int n=0;n<N0;n++)
    F_twiddle_o_errs[n]=F_twiddle_err(n,eta_pi_by_4,logq);
...
int main(int argc, char **argv)
{
  ...
  if(!(infile=fopen(argv[1],"r")))
    fatal_error("Failed to open specification file.");
  if(!(fscanf(infile,"%d\n",&num_files)))
    fatal_error("Expected first line of spec file to define number of files.");
  printf("in %s with num_files=%d\n",argv[0],num_files);
  if(!(infiles=(FILE **) malloc(sizeof(FILE *)*num_files)))
    fatal_error("Failed to allocate memory for infiles.");
  //printf("No. files=%d.\n",num_files);
  for(int n=0;n<num_files;n++)
    {
      if(!(fscanf(infile,"%s\n",fname)))
	fatal_error("Expected each line except first of spec file to containg a filename.");
      //printf("File %d=%s\n",n,fname);
      if(!(infiles[n]=fopen(fname,"rb")))
	{
