/*
  ...
  1.4 Creates own factors on the fly
  1.5 Handles M==1 case specially
  1.6 Same file for odd and even
  Dialect: C++
		...
		FILE *infile)
{
  ...
  if(pr)  // q has a primitive root so nice and easy 1-d FFT
	  ...
	  for(i=1;i<phi_q;i++)  // i=0 corresponds to principal chi
		...
		if(!neg_one)
		  ...
		  if(neg_one)
			...
			//printf("omega[%lu]=",i);print_int_complex_str("",f_omegas[prim_ptr]);
			indices[prim_ptr]=i;
      ...
      for(i=1;i<factors[q].num_facs;i++)  // do the rest on the factors, all will have generators
	{
      ...
      //for(i=0;i<phi_q;i++) printf("offset[%lu]=%lu\n",i,offset_vec[i]); exit(0);
      ...
      while(s_done<num_s)
	{
	  ...
	  if(M>1)
	      ...
	      if(!one_M)
		{
		  ...
		  for(;j<q;j++)
		    if(co_prime(j,q))
		      Fft_vec[offset_vec[i++]]=Fft_vec[offset_vec[0]];
		}
	    }
	  for(i=0;i<factors[q].num_facs;i++)
	    coords[i]=0;
		  ...
		  */
		  ...
		  if(!neg_one)
		      ...
		      if(s_done==0) // first time for this im_s and chi
			{
			  ...
			  f_omegas[prim_ptr]=finish_omega(omegas[i],neg_one);
			  //print_int_complex_str("omega=",f_omegas[prim_ptr]);
			  indices[prim_ptr]=i;
			}
		      zs[neg_ptr*num_s+s_done]=Fft_vec[i]*f_omegas[prim_ptr]+int_complex(err,err);
		      neg_ptr++;
...
int main(int argc, char **argv)
{
  ...
  if(!make_factors(factors,q))
    fatal_error("Error building factors database. Exiting.\n");
  ...
  my_fwrite(&q,sizeof(unsigned int),1,out_file);
  my_fwrite(&n_prims,sizeof(unsigned int),1,out_file);
  my_fwrite(&N,sizeof(int),1,out_file);
  my_fwrite(&eta_even,sizeof(double),1,out_file);
  my_fwrite(&num_s,sizeof(int),1,out_file);
  my_fwrite(&n0,sizeof(int),1,out_file);
  ...
  for(prim=0;prim<n_prims;prim++)
	...
	continue;
      ...
      my_fwrite(&neg_ones[prim],sizeof(bool),1,out_file);
      my_fwrite(&real_p,sizeof(bool),1,out_file);
#ifdef EVEN
      if(neg_ones[prim])
	continue;
#else
	...
	continue;
#endif
      my_fwrite(&indices[prim],sizeof(unsigned int),1,out_file);
      my_fwrite(&f_omegas[prim],sizeof(int_complex),1,out_file);
      //print_int_complex_str("saving",zs[prims[prim]*num_s+1]);
      my_fwrite(&zs[prims[prim]*num_s],sizeof(int_complex),num_s,out_file);
      if(!real_p) // it has a conjugate so save it
	{
	  //printf("Saving composite ind:%d\n",prim1);
	  //printf("prims[%d]=%d\n",prim1,prims[prim1]);

	  my_fwrite(&indices[prim1],sizeof(unsigned int),1,out_file);
	  my_fwrite(&f_omegas[prim1],sizeof(int_complex),1,out_file);
	  //print_int_complex_str("saving",zs[prims[prim1]*num_s+1]);
	  my_fwrite(&zs[prims[prim1]*num_s],sizeof(int_complex),num_s,out_file);
	}
      //else
      //printf("\n");
    }
  ...
  printf("%s successful completion on q=%d n=[%u,%u)\n",argv[0],q,n0,n1);
  return(0);
}
