/*
  ...
  These files are converted by low_q_conv.cpp, then read by f_odd.cpp
...
/* called when wrong arguments passed via command line */
{
  ...
  printf("  (ofname)    - input to f_hat_conv_odd.cpp\n");
  exit(1);
}
		...
		if(neg_one)
		  {
		    ...
		    Fft_vec[i].real+=err;
		    Fft_vec[i].imag+=err;
		    fwrite(&Fft_vec[i],sizeof(int_complex),1,out_file);
		  }
		else
		  {
		    if(s_done==0)
...
int main(int argc, char **argv)
{
  ...
  //printf("Starting...\n");
  make_l_odd(q,num_s,factors,q_n,offset_vec,a_n,omegas,out_file,B,n0,eta_odd,infile);
