//
// low_upsammoreD_assume.cpp
//
// runs upsampling assuming that unknown regions in TZ result
// in pairs of zeros
//
//
// version 1.0
...
int main(int argc, char **argv)
{
  ...
  im_s *im_s_vec;
  int i,j,num_s,index,index2,INDEX;
  ...
  if(argc!=4)
    {
      printf("usage low_upsammoreD_assume <infile> <outfile> <index>\n");
  ...
  while(fread(&q,sizeof(int),1,infile))
	...
	{
	  diff1=rig_calc_zeros_re2(turing_starta,turing_enda,q,gamma,
	...
	{
	  diff1=rig_calc_zeros_cmplx2(turing_starta,turing_enda,q,gamma,
