...
int main(int argc, char **argv)
{
  ...
  im_s *im_s_vec;
  int i,j,num_s,index,index2,n_zeros,t0,t1,Q,INDEX;
  ...
  if(argc!=5)
    {
      printf("usage upsammoreB <infile> <outfile> Q INDEX\n");
  ...
  while(fread(&q,sizeof(int),1,infile))
	...
	{
	  while(sign(re_zs1[turing_start])==UNK)
	    {
	  ...
	  t0=turing_starta*one_over_A;
	  t1=turing_enda*one_over_A;
	  gammaa=int_even(t0,t1);
	  gamma_aa=int_odd(t0,t1);
	...
	{
	  while((sign(re_zs1[turing_start])==UNK)||(sign(re_zs2[turing_start])==UNK))
	    {
	      printf("Moving Turing Start at q=%d index = %d.\n",
		     q,index);
	      turing_starta++;
