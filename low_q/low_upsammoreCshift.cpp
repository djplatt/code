...
#include "../includes/upsample.h"
 ...
 int main(int argc, char **argv)
 {
   ...
   int diff1,cz1,cz2,no_zeros=0,q1,ind1;

   _fpu_rndd();

   if(argc!=5)
     {
       fatal_error("Usage low_upsammoreCshift <infile> <outfile> <q> <index>. Exiting.\n");
     }
   ...
   while(fread(&q,sizeof(unsigned int),1,infile))
       ...
       if(first_time)
	 {
       ...
       turing_starta+=128;
       turing_enda+=128;
       set_d_inter_err(q,(turing_enda+1)*one_over_two_B); // in case of unknown
	   ...
	   if(sign(re_zs1[turing_starta])==UNK)
	     {
	       ...
	       if(sign(re_zs1[turing_starta])==UNK)
		 {
		   printf("Two unknowns in succession at start of TZ. Exiting.\n");
		   exit(1);
		 }
	     }
	   if(neg_one)
	     gamma=int_odd(turing_starta*one_over_two_B,turing_enda*one_over_two_B);
	   else
	     gamma=int_even(turing_starta*one_over_two_B,turing_enda*one_over_two_B);
