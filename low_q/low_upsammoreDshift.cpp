...
int main(int argc, char **argv)
{
  ...
  if(argc!=5)
    {
      printf("usage low_upsammoreDshift <infile> <outfile> <q> <index>\n");
  ...
  while(fread(&q,sizeof(int),1,infile))
      ...
      if(first)
	{
      ...
      turing_starta+=128;
      turing_enda=turing_starta+128;
      printf("TZ=[%d,%d]\n",turing_starta,turing_enda);
      set_d_inter_err(q,(turing_enda+1)*one_over_two_B);
	  ...
	  if(sign(re_zs1[turing_starta])==UNK)
	    {
	      ...
	      if(sign(re_zs1[turing_starta])==UNK)
		{
		  printf("Two unknows at start of TZ. Exiting.\n");
		  exit(1);
		}
	    }
	  if(neg_one)
