...
inline void do_rho(arb_t res1, arb_t res2, arb_t gamma, arb_t omega, arb_t two_alpha, int64_t prec)
{
  ...
  arb_div(tmp,tmp1,tmp2,prec); // exp(-gamma^2/(2alpha))/(1/4+gamma^2)
  ...
  arb_mul(res2,tmp1,tmp,prec);
  arb_neg(res2,res2);
  //printf("Result= ");arb_printd(res2,20);printf("\n");

  arb_mul(tmp1,tmp2,omega,prec);  
  arb_div(tmp,tmp,tmp1,prec); // exp(-gamma^2/(2alpha))/(omega(1/4+gamma^2)^2)
...
int main(int argc, char **argv)
{
  ...
  arb_t gamma,omega,res1,res2,tot1,tot2,del_t,t,two_alpha;
  ...
  printf("test on file %s at omega = %15.14e returned ",(double)atol(argv[3])/atol(argv[4]),argv[2]);
  arb_printd(tot1,20);
  printf(" ");arb_printd(tot2,20);
  arb_add(tot1,tot1,tot2,prec);
  printf(" ");arb_printd(tot1,20);
  fmpz_t a,b,e;
  fmpz_init(a);
  fmpz_init(b);
  fmpz_init(e);
