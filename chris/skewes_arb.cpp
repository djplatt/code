...
inline void do_rho(arb_t res1, arb_t res2, arb_t res3, arb_t res4, arb_t gamma, arb_t omega, arb_t two_alpha, int64_t prec)
{
  ...
  //arb_print_str("Second term = ",res2);
...
int main(int argc, char **argv)
{
  ...
  for(it=0;it<num_its;it++)
      ...
      for(z=zs[0]+1;z<=zs[1];z++)
	{
	  ...
	  do_rho(res1,res2,res3,res4,gamma,omega,two_alpha,prec);
  ...
  arb_add(tot1,tot1,tot3,prec);
  arb_add(tot1,tot1,tot4,prec);
