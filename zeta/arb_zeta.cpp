...
#include "arb_win_zeta.h" // file I/O and stuff
#include "arb_fft.h"
...
arf_t arf_pi,ip_tmp1,ip_tmp2,ip_tmp3,ip_tmp4;
...
void init(double t0,int64_t prec)
{
  ...
  arf_init(arf_pi);
  arb_get_ubound_arf(arf_pi,arb_pi,prec);
  arf_init(ip_tmp2);
  arf_init(ip_tmp1);
  arf_init(ip_tmp2);
  arf_init(ip_tmp3);
...
void inter_t(arb_ptr f_res, arb_ptr fd_res, acb_t *f_vec, arb_ptr t_ptr, int64_t prec)
{
  ...
  if(arb_equal_si(t_ptr,(int64_t) t0_d)) // t_ptr is integral so = a lattice pt
    nearest_t0=t0_d+1.5;
  else
    nearest_t0=t0_d+0.5;
...
long int turing_max(acb_t *f_vec, long int a_ptr, double a, long int b_ptr, double b, int64_t prec)
{
  ...
  //printf("turing_max computed ");arb_printd(tm_res,10);printf("\n");
  ...
  */
  ...
  for(it=0;it<num_its;it++,t0+=step)
      ...
      for(j=0,i=N/2-1;i>=N/2-int_step/2-INTER_SPACING*Ns;j++,i--)
	{
      ...
      for(i=0;i<100;i++)
	{
	  printf("t=%10.e ",t0+(double)i*one_over_A);
