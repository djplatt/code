...
// xi.c
//
// a hacked version of the windowed zeta code
// (win_zeta1.10 to be precise)
// that computes completed zeta function rigorously
// on a lattice of equally spaced points from t0 to t0+step
//
// outputs zeros of xi'(t)
// xi=1/2*(1/2+it)*(-1/2+it)*zeta(1/2+it)*Pi^(-it/2)*gamma(1/4+it/2)
// as doubles
//
// NOT RIGOROUS, but probably not too far from the truth
//
// DJ Platt
...
#include "mpfi.h"
#include "mpfi_io.h"
#include "../includes/mpfi_c.h" // extend mpfi to complexes
#include "../includes/mpfi_fft.h" // mpfi FFT routines
#include "../includes/co_prime.h" // gcd and co_prime
#include "win_zeta.h" // file I/O and stuff
...
#define H ((double) 2089.0/16384.0)//((double) 2089.0/16384.0) // Gaussian width for upsampling
...
#define INTER_B ((double) 0.5/INTER_A)
#else
...
void print_usage()
{
  printf("Usage:- zeta_double prec t_min num_its step outfile.\n");
...
{
  scale_t(tn,t_now);
  if(!is_exact(mpfi_set_fr(res1,tn)))
    fatal_error("tn not exact in bracket_t.");
...
{
  mpfi_log(res,t);
  mpfi_mul_ui(res,res,59);
  mpfi_div_ui(res,res,1000);
  mpfi_set_ui(tm_tmp,2067);
  mpfi_div_ui(tm_tmp,tm_tmp,1000);
  mpfi_add(res,res,tm_tmp);
...
// we take w(t)=xis[t]*exp(-(t-t0)^2/2H^2)*exp(Pi*t0/4)*exp(-Pi*t/4)
// xis[0]=xi(tt0)
void w12(double *w1, double *w2, double t0, double *xis,double tt0)
{
  //printf("in w12 with t0=%20.18e tt0=%20.18e\n",t0,tt0);
  uint64_t n0=round((t0-tt0)*2.0*INTER_B)-Ns; // starting point
  double t=n0/(2.0*INTER_B)+tt0; // t at that sample point
  //printf("n0=%lu xi(%20.18e)=%20.18e\n",n0,t,xis[n0*INTER_SPACING]);
  uint64_t n,i;

  double w0=0.0;

  w1[0]=0.0;w2[0]=0.0; // w1 will contain w'(t0), w2 gets w''(t0)

  // do the sum, starting at n=n0,n0+INTER_SPACING ...
  double n2Bt=(double) n0-(t0-tt0)*2.0*INTER_B;
  for(n=n0;n<=n0+Ns*2;n++,t+=INTER_A,n2Bt++)
    {
      double dt=t0-t;
      double wnA=xis[n*INTER_SPACING]*exp(M_PI*(t0-t)/4.0-dt*dt/(2.0*H*H));
      //printf("W(%20.18e)=%20.18e n-2Bt=%20.18e\n",t,wnA,n2Bt);
      if(n2Bt==0.0)
	{
	  w1[0]+=0.0;
	  w2[0]+=-4.0/3.0*M_PI*M_PI*INTER_B*INTER_B;
	  continue;
	}
      double s=sin(n2Bt*M_PI);
      double c=cos(n2Bt*M_PI);
      //printf("n=%lu xi(n/2B)=%20.18e W(n/2B)=%20.18e\n",n,xis[n],wnA);
      double term=wnA*s/(M_PI*n2Bt);
      //printf("adding %20.18e to w\n",term);
      w0+=term;
      term=wnA*(2.0*INTER_B*s/(M_PI*n2Bt*n2Bt)-2.0*INTER_B*c/n2Bt);
      //printf("adding %20.18e to w'\n",term);
      w1[0]+=term;
      term=wnA*4.0*INTER_B*INTER_B/n2Bt*(2*s/(M_PI*n2Bt*n2Bt)-2*c/n2Bt-M_PI*s);
      //printf("adding %20.18e to w''\n",term);
      w2[0]+=term;
    }
  //printf("w(%20.18e)=%20.17e\n",t0,w0);
}
void w1(double *w1, double t0, double *xis,double tt0)
{
  //printf("in w1 with t0=%20.18e tt0=%20.18e\n",t0,tt0);
  uint64_t n0=round((t0-tt0)*2.0*INTER_B)-Ns; // starting point
  double t=n0/(2.0*INTER_B)+tt0,term; // t at that sample point
  //printf("n0=%lu xi(%20.18e)=%20.18e\n",n0,t,xis[n0*INTER_SPACING]);
  uint64_t n,i;

  w1[0]=0.0; // w1 will contain w'(t0)
...
double wbinary_chop(double t0, double t1, double t2, double *xis, double tt0)
{
  double x0,x2;
  w1(&x0,t0,xis,tt0);
  w1(&x2,t2,xis,tt0);

  if(((x0<0.0)&&(x2<0.0))||((x0>0.0)&&(x2>0.0)))
    {
      double x1;
      w1(&x1,t1,xis,tt0);
      printf("Fatal error in wbinary chop. Both end points have same sign. Exiting.\n");
      printf("%20.18e %20.18e %20.18e\n",x0,x1,x2);
      exit(0);
    }


  uint64_t count;
  for(count=0;count<=NUM_CHOPS;count++)
    {
      double x1;
      w1(&x1,t1,xis,tt0);
      //printf("iteration %lu %20.18e %20.18e %20.18e %20.18e %20.18e %20.18e\n",count,t0,t1,t2,x0,x1,x2);
      if(((x1<0.0)&&(x0>0.0))||((x1>0.0)&&(x0<0.0))) // zero between t0 and t1
	{
	  x2=x1;t2=t1;
	  t1=(t0+t1)/2.0;
	}
      else // between t1 and t2
	{
	  x0=x1;t0=t1;
	  t1=(t1+t2)/2.0;
	}
...
// use N-R to find zero of xi'
double xi_zero(double x0, double *xis, double tt0)
{
  //printf("starting N-R at x0=%20.18e\n",x0);
  double w1,w2,x1;
  w12(&w1,&w2,x0,xis,tt0);
  //printf("w'(%20.18e)=%20.18e w''(.)=%20.18e\n",x0,w1,w2);
  x1=x0-w1/w2;
  //printf("x1=%20.18e x0=%20.18e\n",x1,x0);
  while(fabs(x1-x0)>EPS)
    {
      x0=x1;
      w12(&w1,&w2,x0,xis,tt0);
      //printf("w'(%20.18e)=%20.18e w''(%20.18e=%20.18e\n",x0,w1,x0,w2);
      x1=x0-w1/w2;
      //printf("x1=%20.18e x0=%20.18e\n",x1,x0);
    }
  return(x1);
...
int main(int argc, char **argv)
{
  ...
  if(step<=0)
    print_usage();
  t0=t0+step/2.0;
  if((t0>T0_MAX)||(t0<T0_MIN))
    fatal_error("t0 outside limits.");
  if(prec<=OP_ACC+1+log(T0_MAX+step/2.0)/log(2.0)) // enough bits to make 2^(OP_ACC+1)*t0 exact ?
  fatal_error("Not enough operating accuracy for this size of t0.");
  ...
  double *xis=(double *)malloc(sizeof(double)*(int_step+1+INTER_SPACING*Ns*2));
  if(!xis)
    fatal_error("Couldn't allocate memory for xis.");
  ...
  for(it=0;it<num_its;it++,t0+=step)
      ...
      for(j=0,i=N/2-1;i>=N/2-int_step/2-INTER_SPACING*Ns;j++,i--)
	{
      ...
      // fill xis with w(t)=xi[t]*exp(Pi*t/4)
      // xis[0] contains w(tt0)
      double tt0=t0-step/2-INTER_SPACING*Ns*one_over_A;
      for(t=tt0,j=0,i=N/2-int_step/2-INTER_SPACING*Ns;i<=N/2+int_step/2+INTER_SPACING*Ns;t+=one_over_A,i++,j++)
	{
	  xis[j]=-mpfi_get_d(f_vec[i]->re)*0.5*(t*t+0.25);
	  //printf("%20.18e %20.18e\n",t,xis[j]);
	  //mpfi_print_str("",f_vec[N/2+i]->re);
	}
      uint64_t ptr=INTER_SPACING*Ns-1;
      dir_t last_dir=UNK;
      double efac=exp(M_PI*one_over_A/4.0);
      while(last_dir==UNK)
	{
	  ptr++;
	  last_dir=double_dir(xis[ptr]*efac,xis[ptr+1]);
	}
      //printf("Starting with ptr=%lu and direction %s.\n",ptr,(last_dir==UP ? "UP" : "DOWN"));
      uint64_t count=0;
      dir_t this_dir=last_dir;
      while(TRUE)
	{
	  while((this_dir==last_dir)||(this_dir==UNK))
	    {
	      ptr++;
	      if(ptr>Ns*INTER_SPACING+int_step)
		break;
	      this_dir=double_dir(xis[ptr]*efac,xis[ptr+1]);
	    }
	  //printf("Now at ptr=%lu and direction %s.\n",ptr,(last_dir==UP ? "UP" : "DOWN"));

	  if(ptr>Ns*INTER_SPACING+int_step)
	    break;
	  //printf("Looking for zero near %20.18e\n",tt0+((double)ptr)*one_over_A);
	  double t2=tt0+((double)(ptr+1))*one_over_A;
	  double t1=t2-2.0*one_over_A;
	  //printf("Doing stat point %lu with ptr=%lu t1=%20.18e t2=%20.18e\n",++count,ptr,t1,t2);
	  //printf("xis = %20.18e %20.18e %20.18e\n",xis[ptr-1]*efac*efac,xis[ptr]*efac,xis[ptr+1]);
	  //printf("Directions %s %s\n",dir_strings[last_dir],dir_strings[this_dir]);
	  double zero=xi_zero(t1+one_over_A,xis,tt0);//xi_zero(t2-one_over_A,xis,tt0);
	  if((zero<t1)||(zero>t2))
	    {
	      printf("N-R wandered off somewhere. Resorting to binary chop.\n");
	      printf("We bracketed %20.18e %20.18e but got %20.18e\n",t1,t2,zero);
	      zero=wbinary_chop(t1,t1+one_over_A,t2,xis,tt0);
	    }
	  printf("Zero returned was     %20.18e\n",zero);
	  last_dir=this_dir;
	}

      this_time=time(NULL);
      printf("Time to output to file = %G seconds.\n",difftime(this_time,last_time));
      last_time=this_time;
