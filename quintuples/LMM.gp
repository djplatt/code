...
generate_cs_ds(u,v,x0,y0,D,a,b,b1)=
{
   ...
   while(c<lwb,
      if((type(c)=="t_INT")&&(c>b),
         printf("Adding c %d for a %d and b% d\n",c,a,b);
         cs[cptr]=[c];
         cptr++);
      t=x*u+y*v*D;
      y=x*v+y*u;
...
solveit4(r,a,b)={
   ...
   if(m1[1]==-1,
      ...
      for(y=L1,L2,Sq=D*y^2-N;
         if(issquare(Sq),
            x=floor(sqrt(Sq));
            print("Found -1 case ",m1[2]," ",m1[3]," ",x," ",y," ",D," ",a," ",b," ",b1);
   ...
   for(y=0,L2,Sq=N+D*y^2;
      if(issquare(Sq),
         ...
         generate_cs_ds(m1[2],m1[3],x,y,D,a,b,b1);
         generate_cs_ds(m1[2],-m1[3],x,y,D,a,b,b1)));
   /*for(i=1,cptr-1,printf("%d %d %d %d\n",r,a,b,cs[i][1]));*/
   test_ds(r,a,b);
}

solve_minus_ones(r,a,b)={
   local(g,a1,b1,D,sfd,N1,N,sN,sD,L1,L2,x,Sq);
   for(i=1,C_LIM,cs[i]=[0]);
   for(i=1,D_LIM,ds[i]=[0]);
   cptr=1;dptr=1;
   g=gcd(a,b);\\
