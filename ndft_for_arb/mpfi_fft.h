...
inline void mpfi_c_setup(int prec)
{
...
void bluesteinfft(mpfi_c_t *x, uint64_t len, uint64_t stride)
{
  ...
  if(B[len].conv_size==0) // not seen this size before
	          ...
	          // so construct the ws
	{
      ...
      else // we have seen this length before, so just point to ws
	w=ws[l2];
    }
  ...
  for(i=1,s=stride;i<len;i++,s+=stride)
    {
      ...
      mpfi_c_mul(A[i],x[s],dft_spare[0]);
  ...
  for(i=1,s=stride;i<len;i++,s+=stride)
    {
      mpfi_c_conj(dft_spare[0],B[len].b[i]);
      mpfi_c_mul(x[s],dft_spare[0],A[i]);
    }
}
...
// set x[m]=sum n=0,len-1 x[n]*e(-mn/len)
