...
int main(int argc, char **argv)
{
  ...
  uint64_t first_h=STEP_SIZE>>(n+1); // need 1/2 gap at start
  ...
  // here last_h was a good proth prime, or just before a Pari prime
  // this_h is beyond h1-h0
  this_h=h1-h0;
  while(this_h>last_h)
    {
      if(!test_bit(this_h,bits))
	{
	  this_h--;
	  continue;
	}
