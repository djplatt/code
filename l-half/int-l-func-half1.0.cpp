      ...
      FILE *out_file)
{
  ...
  if(pr)  // q has a primitive root so nice and easy 1-d FFT
  {
      ...
      for(i=0;i<phi_q;i++)
	{
	  ...
	  */
	  //printf("L(1/2,chi_%ld_%ld=",q,i);print_int_complex_str("",z*q_minus_half);
	  if(contains_zero(z.real)&&contains_zero(z.imag))
	    {
	      printf("L(1/2,chi) might contain zero for q %ld index %ld\n",q,i);
	      print_int_complex_str("L(1/2,chi)=",z);
	    }
      ...
      for(i=0;i<phi_q;i++)
	{
	  ...
	  for(j=0;j<factors[q].num_facs;j++)
	      ...
	      if((i==101)&&(s_done==123))
		{
	      ...
	      */
	      //printf("L(0.5,chi_%ld_%ld)=",q,i);print_int_complex_str("",z*q_minus_half);
	      if(contains_zero(z.real)&&contains_zero(z.imag))
		{
		  printf("L(1/2,chi) might contain zero for q %ld index %ld\n",q,i);
		  print_int_complex_str("L(1/2,chi)=",z);
  ...
  for(q=q_start;q<=q_end;q++)
	...
	if(co_prime(num,q))
	  ...
	  // figure out nearest row in lattice
	    ...
	    // out_val now contains Hurwitz(s,num/q)
