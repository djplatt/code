...
double hurwitz_half(double alpha)
{
  ...
  printf("hurwitz returning %20.18e\n",res);
  return(res);
}
...
int main(int argc, char ** argv )
	 ...
	 */
	 ...
	 for(int i=0;i<PHYSICAL_N;i++)
	   printf("in[%ld]=%f\n",i,in[i]);
	 */
	 double sqrt_q=sqrt((double) P);
	 //fwrite(&P,sizeof(long unsigned int),1,outfile);
	 //fwrite(in,sizeof(double),PHYSICAL_N,outfile);
	 for(long int i=0;i<LOGICAL_N/2+1;i++)
	   printf("L(1/2,chi_%ld)= %15.13lg +  %15.13lg i\n",i,in[i*2]/sqrt_q,in[i*2+1]/sqrt_q);
	 fftw_destroy_plan(p);
